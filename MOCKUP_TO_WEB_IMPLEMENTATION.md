...
- **통합 그리드**: AI 랭킹 + 커뮤니티 (좌측 3/4) + 사이드바 (우측 1/4)
- **그리드 구조**: `grid-cols-1 lg:grid-cols-4` → 메인 `lg:col-span-3`, 사이드바 `lg:col-span-1`
...
**제목 및 설명**:
- 제목: "🤖 AI 평점 랭킹"
- 설명: "AI가 공개된 데이터를 활용하여 객관적으로 평가한 정치인 평점 순위 (TOP 10)"

**테이블 구조**:
- 10개 행 (TOP 10)
- 컬럼: 순위, 이름, 신분, 정당, 지역, Claude 평점, GPT/Gemini/Grok/Perp (추후 표시 예정)
...
- 2열: 6~10번
- 3열: 11~15번
- 각 항목: 순위 뱃지, 제목, 조회수, 댓글수, 추천수, 공유하기 버튼

### 5. 사이드바 (우측 1/4)
**시작 라인 정렬**:
- `mt-4` 제거하여 AI 평점 랭킹과 같은 라인에서 시작

**포함 섹션** (순서대로 8개):
1. 📊 정치인 등록 현황
2. 📈 평점 급상승 정치인 (TOP 3)
3. 🏆 명예의 전당
4. 👤 내 프로필 (레벨, XP, 작성글, 받은 추천)
5. ⚡ 실시간 통계
6. 💬 최근 댓글
7. 🔗 연결 서비스 (법률자문, 홍보, 컨설팅)
8. 📺 광고 영역
...
## 체크리스트
- [x] Header: 브랜드명 좌측, 메뉴 우측
- [x] AI 평점 랭킹: Claude 평점 소수점 표시, 회원 평점 별점만
- [x] 정치인 최근 글: 3열 × 3줄 = 9개, 공유하기 버튼 포함
- [x] 실시간 인기글: 3열 × 5줄 = 15개, 공유하기 버튼 포함
- [x] 사이드바: 8개 위젯 (등록 현황 + 급상승 + 명예의 전당 + 프로필 + 통계 + 댓글 + 연결서비스 + 광고)
- [x] Footer: 4개 메뉴 횡렬 중앙 배치
- [x] 전체 페이지: max-w-7xl 여백 적용
- [x] 통합 그리드: 3/4 + 1/4 레이아웃
- [x] 로컬 빌드 성공
- [x] Vercel 배포 성공

## 최근 업데이트 (2025-10-20)
### 텍스트 개선
- 검색 placeholder: "정치인 이름, 신분, 직종, 지역, 정당으로 검색..."
- "AI 평가 랭킹" → "AI 평점 랭킹"
- "공개된 데이터를 활용하여" 명시
- "당" → "정당" (일관성)
- "평점 급상승 중인 정치인" → "평점 급상승 정치인"
- 연결 서비스: "법률 자문" → "법률자문", 설명 간결화

### 레이아웃 최적화
- 2/3 + 1/3 → 3/4 + 1/4 그리드로 변경
- AI 랭킹 테이블 가독성 향상
- 메인 컨텐츠 영역 확대
