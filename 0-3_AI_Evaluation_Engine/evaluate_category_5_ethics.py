...
# 정치인 정보
POLITICIAN_NAME = "오세훈"
POLITICIAN_ID = "272"  # 입력받은 ID (UUID로 변환 필요할 수 있음)
CATEGORY_NUM = 5
CATEGORY_NAME = "윤리성"
AI_NAME = "Claude"
...
def insert_collected_data(conn, politician_id, item_num, data_point):
    ...
    try:
        ...
        """, (
            ...
            CATEGORY_NUM,
            item_num,
            data_point.get('data_type', 'news'),
            data_point['title'],
            data_point['content'],
            data_point.get('url', ''),
            data_point['rating'],
            data_point.get('reliability', 0.8),
            datetime.now()
        ))
        conn.commit()
        return True
...
def main():
    ...
    for item in ITEMS:
        ...
        for dp in data_points:
            if insert_collected_data(conn, politician_uuid, item_num, dp):
                inserted_count += 1
