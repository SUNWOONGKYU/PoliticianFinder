...
def insert_data_to_db():
    ...
    try:
        ...
        print(f"\n카테고리 {CATEGORY_NUM} ({CATEGORY_NAME}) 항목별 통계:")
        print("-" * 80)
        print(f"{'항목':<8} {'데이터 수':<12} {'평균 Rating':<15} {'평균 신뢰도':<15}")
        print("-" * 80)

        total_data = 0
        total_rating_sum = 0
        total_reliability_sum = 0

        for row in results:
            cat_num, item_num, data_count, avg_rating, avg_reliability = row
            print(f"{item_num:<8} {data_count:<12} {float(avg_rating):+.2f}{'':<12} {float(avg_reliability):.2f}")
            total_data += data_count
            total_rating_sum += float(avg_rating) * data_count
            total_reliability_sum += float(avg_reliability) * data_count

        print("-" * 80)
        overall_avg_rating = total_rating_sum / total_data if total_data > 0 else 0
        overall_avg_reliability = total_reliability_sum / total_data if total_data > 0 else 0
        print(f"{'전체':<8} {total_data:<12} {overall_avg_rating:+.2f}{'':<12} {overall_avg_reliability:.2f}")
        print("-" * 80)
        ...
        print(f"- 평균 Rating: {overall_avg_rating:+.2f}")
        print(f"- 평균 신뢰도: {overall_avg_reliability:.2f}")
