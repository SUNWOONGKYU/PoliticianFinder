...
사용법:
    # 시뮬레이션 (DRY RUN)
    python adjust_v50.py --politician_id={ID} --politician_name="{이름}"

    # 실제 조정 실행
    python adjust_v50.py --politician_id={ID} --politician_name="{이름}" --no-dry-run

    # 특정 AI만 조정
    python adjust_v50.py --politician_id={ID} --politician_name="{이름}" --ai=Gemini --no-dry-run

상세 가이드:
    instructions/V50_검증후조정_가이드.md
"""
...
# .env 로드 (다중 경로 시도)
env_paths = [
    V50_DIR.parent.parent / '.env',  # 0-3_AI_Evaluation_Engine/.env
    V50_DIR.parent / '.env',          # 설계문서_V7.0/.env
    V50_DIR / '.env',                 # V50/.env
    Path('.env')                      # 현재 디렉토리
]
...
# 목표 균형 (V50 3채널: Gemini 40% + Grok-X 10% + Naver 50%)
MIN_PER_CATEGORY = 50   # 전체 최소 목표 (카테고리당)
MAX_PER_CATEGORY = 60   # 전체 버퍼 목표 (카테고리당)

# 채널별 목표
GEMINI_MIN_PER_CATEGORY = 40   # Gemini 최소 (40%)
GEMINI_MAX_PER_CATEGORY = 48   # Gemini 버퍼 (40%)
GROK_X_MIN_PER_CATEGORY = 10  # Grok-X 최소 (10%)
GROK_X_MAX_PER_CATEGORY = 12  # Grok-X 버퍼 (10%)
NAVER_MIN_PER_CATEGORY = 50   # Naver 최소 (50%)
NAVER_MAX_PER_CATEGORY = 60   # Naver 버퍼 (50%)

MIN_PER_AI = 400       # Gemini/Grok-X 최소 (40개/10개 × 10)
MAX_PER_AI = 600       # Naver 버퍼 목표 (60 × 10)

# 채널별 AI명
CHANNEL_AIS = ['Gemini', 'Grok-X', 'Naver']
CHANNEL_TARGETS = {
    'Gemini': (GEMINI_MIN_PER_CATEGORY, GEMINI_MAX_PER_CATEGORY),
    'Grok-X': (GROK_X_MIN_PER_CATEGORY, GROK_X_MAX_PER_CATEGORY),
    'Naver':  (NAVER_MIN_PER_CATEGORY, NAVER_MAX_PER_CATEGORY),
}
...
def analyze_current_status(politician_id: str) -> Dict:
    ...
    for ai in CHANNEL_AIS:
        ch_min, ch_max = CHANNEL_TARGETS[ai]
        for cat in CATEGORIES:
            ...
            # 상태 판단 (채널별 목표 사용)
            if count > ch_max:
                status = f"초과 ({count - ch_max}개)"
                issues.append(('overflow', ai, cat, count))
            elif count < ch_min:
                status = f"부족 ({ch_min - count}개)"
    ...
    for ai in CHANNEL_AIS:
        ...
        ai_min = ch_min * len(CATEGORIES)
        ai_max = ch_max * len(CATEGORIES)
        status = "OK" if ai_min <= total <= ai_max else ("초과" if total > ai_max else "부족")
        print(f"  {ai}: {total}/{ai_max} ({status})")

    grand_total = sum(total_by_ai.values())
    total_max = sum(ch_max * len(CATEGORIES) for _, (_, ch_max) in CHANNEL_TARGETS.items())
    print(f"\n전체 합계: {grand_total}/{total_max}")
...
def trim_overflow(politician_id: str, ai: str, category: str, current_count: int, dry_run: bool = True) -> int:
    """
    Step 2: 초과 처리
    채널별 MAX 초과 카테고리 → 시간순 삭제 (오래된 것부터)
    채널별 목표: Gemini 48, Grok-X 12, Naver 60

    Returns:
        삭제할 개수 (실제 삭제는 dry_run=False일 때만)
    """
    _, ch_max = CHANNEL_TARGETS.get(ai, (MIN_PER_CATEGORY, MAX_PER_CATEGORY))
    excess = current_count - ch_max
    if excess <= 0:
        return 0
...
def trigger_recollection(politician_id: str, politician_name: str, ai: str, category: str,
    ...
    try:
        if ai == 'Gemini':
            # Gemini REST API 재수집
            # Note: collect_gemini_v50.py --target 기본값 48 (40%)
            # 재수집 시 자동으로 부족분만 수집
            script_path = V50_DIR / 'scripts' / 'collect_gemini_v50.py'
            cmd = [
                sys.executable,
                str(script_path),
            ...
            # Grok-X 재수집 (V50/scripts/collect_grok_x_v50.py)
            collect_grok_script = V50_DIR / 'scripts' / 'collect_grok_x_v50.py'
            cmd = [
                sys.executable,
                str(collect_grok_script),
                '--politician_id', politician_id,
                '--politician_name', politician_name,
                '--category', category
            ]
            ...
            # Naver API 재수집 (V50/scripts/collect_naver_v50.py)
            collect_naver_script = V50_DIR / 'scripts' / 'collect_naver_v50.py'
            cmd = [
                sys.executable,
...
def _verify_not_error(politician_id: str, politician_name: str) -> Dict:
    ...
    try:
        ...
        # 조건: API 정상 AND 검색결과 존재 AND 기간 내 데이터 0개 AND 필터 정상
...
def validate_balance(politician_id: str) -> Tuple[bool, Dict]:
    ...
    for ai in CHANNEL_AIS:
        ...
        for cat in CATEGORIES:
            ...
            if count < ch_min or count > ch_max:
                all_balanced = False
    ...
    for ai in CHANNEL_AIS:
        ...
        ai_min = ch_min * len(CATEGORIES)
        ai_max = ch_max * len(CATEGORIES)
        if total_by_ai[ai] < ai_min or total_by_ai[ai] > ai_max:
            all_balanced = False
...
def adjust_data(politician_id: str, politician_name: str, target_ai: str = None, dry_run: bool = True) -> bool:
    ...
    Returns:
        균형 달성 여부
    """
    print_header(f"V50 검증 후 조정 - {politician_name}")
    ...
    # 조정 대상 AI 필터링
    ais_to_adjust = [target_ai] if target_ai else CHANNEL_AIS
    ...
    # after_stats 초기화: 조정 라운드가 실행되기 전 print_final_report에 도달하는 경우 방지
    after_stats = {}
...
def main():
    parser = argparse.ArgumentParser(
        description='V50 검증 후 조정 스크립트 (Phase 2-2)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
사용 예시:
  # 시뮬레이션 (DRY RUN)
  python adjust_v50.py --politician_id=d0a5d6e1 --politician_name="조은희"

  # 실제 조정 실행
  python adjust_v50.py --politician_id=d0a5d6e1 --politician_name="조은희" --no-dry-run

  # Gemini만 조정
  python adjust_v50.py --politician_id=d0a5d6e1 --politician_name="조은희" --ai=Gemini --no-dry-run
...
상세 가이드:
  instructions/V50_검증후조정_가이드.md
    ...
    parser.add_argument('--ai', choices=['Gemini', 'Grok-X', 'Naver'], help='특정 채널만 조정 (선택)')
