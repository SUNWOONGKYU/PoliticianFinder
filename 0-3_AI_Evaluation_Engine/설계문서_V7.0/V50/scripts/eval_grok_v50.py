# -*- coding: utf-8 -*-
"""
V50 Grok 평가 (xAI API Direct, requests)

xAI API를 requests.post로 직접 호출하여 수집 데이터를 Grok으로 평가합니다.

모델: grok-3-mini (V50)
방식: requests.post (순수 Python HTTP, curl subprocess 아님)
테이블: collected_data_v50, evaluations_v50
배치 크기: 25개
instructions 경로: instructions/3_evaluate_v50/

V40 대비 변경사항:
- curl subprocess -> requests.post (순수 Python HTTP)
- 모델: grok-3 -> grok-3-mini
- V40 의존성(common_eval_saver, phase_tracker) 제거 -> 자체 구현

사용법:
    python eval_grok_v50.py --politician_id=ID --politician_name="이름" --category=expertise
    python eval_grok_v50.py --politician_id=ID --politician_name="이름" --category=all
"""
...
def extract_response_text(api_response: dict) -> str:
    ...
    content_field = first_output.get('content', '')
...
def call_grok_api(prompt: str, timeout: int = 120) -> str:
    """xAI API를 requests.post로 호출, 응답 텍스트 반환"""
    if not XAI_API_KEY:
        print('  오류: XAI_API_KEY 환경변수가 없습니다.', file=sys.stderr)
        return ''
    ...
    try:
        ...
        if resp.status_code != 200:
            print(f'  Grok API 오류: {resp.status_code} {resp.text[:300]}', file=sys.stderr)
            return ''
        ...
        if not text:
            print(f'  Grok 응답 텍스트 없음. 응답 구조: {str(api_response)[:300]}', file=sys.stderr)
            return ''

        return text

    except requests.Timeout:
        print(f'  Grok API 타임아웃 ({timeout}초)', file=sys.stderr)
        return ''
    except Exception as e:
        print(f'  Grok API 오류: {e}', file=sys.stderr)
        return ''
...
def parse_json_response(text: str) -> list:
    ...
    # JSON 객체 내 evaluations 필드 처리 (일부 모델이 래핑하는 경우)
    if '{' in text:
        start = text.find('{')
        end = text.rfind('}') + 1
        try:
...
def evaluate_batch_grok(items: list, politician_name: str, category: str, instruction_content: str = '') -> list:
    """Grok API로 배치 평가"""
...
def main():
    parser = argparse.ArgumentParser(description='V50 Grok 평가 (xAI API Direct, requests)')
    ...
    print('=' * 70)
    print(f'V50 Grok 평가: {args.politician_name} (모델: {GROK_MODEL})')
    print('=' * 70)
    ...
    for cat in categories:
        ...
        for i in range(0, len(unevaluated), args.batch_size):
            ...
            results = evaluate_batch_grok(batch, args.politician_name, cat, instruction_content)
            if results:
    ...
    print('\n' + '=' * 70)
    print(f'V50 Grok 평가 완료: 총 {total_saved}개 저장')
    print('=' * 70)
