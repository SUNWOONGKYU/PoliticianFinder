# -*- coding: utf-8 -*-
"""
V50 ChatGPT 평가 (OpenAI API Direct)

OpenAI SDK를 사용하여 수집 데이터를 ChatGPT로 평가합니다.
...
V40 대비 변경사항:
- 모델: o4-mini -> gpt-4o-mini
- V40 의존성(common_eval_saver, phase_tracker) 제거 -> 자체 구현

사용법:
    python eval_chatgpt_v50.py --politician_id=ID --politician_name="이름" --category=expertise
    python eval_chatgpt_v50.py --politician_id=ID --politician_name="이름" --category=all
"""
...
def evaluate_batch_chatgpt(items: list, politician_name: str, category: str,
                            instruction_content: str = '', client: OpenAI = None) -> list:
    """gpt-4o-mini로 배치 평가"""
    if client is None:
    ...
    try:
        response = client.chat.completions.create(
            model=CHATGPT_MODEL,
            messages=[
                {"role": "system", "content": "당신은 정치인 평가 전문 AI입니다. 반드시 JSON 배열 형식으로만 답변하세요."},
        ...
        if '[' in text:
            ...
            raw_results = json.loads(text[start:end])
        else:
            print('  응답에 JSON 배열 없음', file=sys.stderr)
            return []
        ...
        print(f'  ChatGPT API 오류: {e}', file=sys.stderr)
        return []
...
def main():
    parser = argparse.ArgumentParser(description='V50 ChatGPT 평가 (OpenAI API Direct)')
    ...
    print('=' * 70)
    print(f'V50 ChatGPT 평가: {args.politician_name} (모델: {CHATGPT_MODEL})')
    print('=' * 70)
    ...
    for cat in categories:
        ...
        for i in range(0, len(unevaluated), args.batch_size):
            ...
            results = evaluate_batch_chatgpt(batch, args.politician_name, cat, instruction_content, client)
            if results:
    ...
    print('\n' + '=' * 70)
    print(f'V50 ChatGPT 평가 완료: 총 {total_saved}개 저장')
    print('=' * 70)
