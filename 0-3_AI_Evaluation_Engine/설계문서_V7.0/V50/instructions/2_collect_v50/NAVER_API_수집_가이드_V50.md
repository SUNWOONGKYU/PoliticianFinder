...
V50 Naver 수집은 **Naver Search API 직접 호출** 방식입니다.
...
- collected_data_v50 테이블에 자동 저장

**스크립트:**
- `collect_naver_v50.py --ai=Naver` (AI 엔진 루트 폴더에 위치)
...
| 역할 | 수행자 | 내용 |
|------|--------|------|
| 전체 프로세스 | collect_naver_v50.py | API 호출, 응답 파싱, DB 저장 |
| 검색 실행 | Naver Search API | 6개 엔드포인트 검색 (news, blog, cafearticle, kin, webkr, doc, encyc) |
| 사용자 | 스크립트 실행 | `python collect_naver_v50.py --ai=Naver --politician_id=ID --politician_name="이름"` |
...
Naver 합계:     50개 (버퍼 60개) — 전체의 50%

V50 전체 배분:
- Gemini API: OFFICIAL 30 + PUBLIC 10 = 40개 (40%)
- Grok(X):    PUBLIC only 12개 (10%, 전부 free)
- Naver NCP:  OFFICIAL 10 + PUBLIC 40 = 50개 (50%)
→ 합계: 기본 102개/카테고리, 버퍼 포함 최대 120개
...
```bash
# collect_naver_v50.py는 AI 엔진 루트에 위치
...
| 파일 | 경로 | 용도 |
|------|------|------|
| 정치인 정보 | `설계문서_V7.0/V50/instructions/1_politicians/{이름}.md` | 이름, 정당, 직책, 지역구 |
| 수집 프롬프트 | `설계문서_V7.0/V50/instructions/2_collect_v50/prompts/naver_official.md` | OFFICIAL 수집 템플릿 |
| 수집 프롬프트 | `설계문서_V7.0/V50/instructions/2_collect_v50/prompts/naver_public.md` | PUBLIC 수집 템플릿 |
| 카테고리 지침 | `설계문서_V7.0/V50/instructions/2_collect_v50/cat01~10_*.md` | 카테고리별 수집 기준 |
...
**결과 (V50 기준):**
- Gemini API: 40개/카테고리 (OFFICIAL 30 + PUBLIC 10)
- Grok(X): 12개/카테고리 (PUBLIC only, 전부 free)
- Naver: 50개/카테고리 (OFFICIAL 10 + PUBLIC 40)
- 총 102개/카테고리 × 10개 카테고리 ≈ 1,020개
...
**실제 구현 (collect_naver_v50.py:843-848):**
```python
params = {
...
**처리 로직 (collect_naver_v50.py:862-904):**
```python
for item in items:
    # 1. HTML 태그 제거
...
**정치인 이름 확인 (collect_naver_v50.py:884-886):**
```python
...
**동명이인 구분 (collect_naver_v50.py:780-795):**
```python
# 정치인명에서 당명, 직책 추출
# 예: "국민의힘 조은희 국회의원 (양천을)"
#     → actual_name: "조은희"
...
**파일 다운로드 페이지 제외 (collect_naver_v50.py:889-891):**
```python
skip_extensions = ['.xlsx', '.xls', '.csv', '.hwp', '.pdf', '.zip']
if any(link.lower().endswith(ext) for ext in skip_extensions):
    continue
...
**날짜 정규화 (collect_naver_v50.py:894):**
```python
...
**검증 단계에서 확인:**
- `validate_v50_fixed.py`가 기간 제한 위반 자동 탐지
...
**동작:**
- collected_data_v50 테이블에서 기존 수집 개수 확인
- 120개 미만 카테고리만 추가 수집
- 버퍼 포함 최대 120개
...
**해결:**
```bash
# validate_v50_fixed.py 실행 (중복 자동 제거)
cd 설계문서_V7.0/V50/scripts/core
python validate_v50_fixed.py \
  --politician_id={ID} \
  --no-dry-run
```
...
**V50 전체 비용 (Gemini + Grok + Naver):**
```
Gemini 2.0 Flash-Lite API: $0 (무료 Tier)
Grok(X) Live Search API:   $0 (무료 Tier)
...
### 11.1 내부 문서
- `V50_기본방침.md` - V50 핵심 규칙 (수집 배분, 등급 체계)
- `V50_전체_프로세스_가이드.md` - 7단계 프로세스
- `TERMINOLOGY.md` - 공식 용어 정의
- `instructions/2_collect_v50/prompts/naver_official.md` - OFFICIAL 템플릿
- `instructions/2_collect_v50/prompts/naver_public.md` - PUBLIC 템플릿
- `instructions/2_collect_v50/cat01~10_*.md` - 카테고리별 수집 기준

### 11.2 관련 스크립트
```
0-3_AI_Evaluation_Engine/
├── collect_naver_v50.py                           # Naver 수집 메인 스크립트
├── duplicate_check_utils.py                  # 중복 체크 유틸
└── 설계문서_V7.0/V50/
    ├── scripts/
    │   └── core/
    │       └── validate_v50_fixed.py        # 수집 검증 및 재수집
    └── instructions/
        └── 2_collect/
            ├── prompts/
            │   ├── naver_official.md
...
**실제 구현 기준**: `collect_naver_v50.py` (V50 버전)
