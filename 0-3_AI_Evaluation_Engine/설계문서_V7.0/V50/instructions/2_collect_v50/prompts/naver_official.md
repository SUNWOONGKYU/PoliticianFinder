# Naver OFFICIAL 수집 프롬프트 템플릿
# 배분: 10개 (버퍼 20% 포함 12개)
# 센티멘트: negative 1개 / positive 1개 / free 10개 (버퍼 포함)
# 이 파일은 collect_naver_v50.py 또는 오케스트레이션 에이전트가 로드합니다.
#
# [플레이스홀더 목록]
...
# 1. .go.kr 도메인 OFFICIAL 데이터 수집
# 2. 국내 공식 기관 데이터에 최적화
# 3. JSON 응답에서 title, link, description, postdate 추출
# 4. Python 스크립트가 자동 실행 (수동 작업 없음)
#
# [빠른 참조]
# 이 파일은 collect_naver_v50.py가 자동으로 읽어서 플레이스홀더를 치환합니다.
# 수동 편집 불필요. 프로세스:
#   1. collect_naver_v50.py 실행 → 이 템플릿 로드
#   2. {politician_full}, {category_kr} 등 자동 치환
#   3. Naver Search API 호출 → 결과 JSON 수신
#   4. collected_data_v50 테이블에 저장
#
# 목표: OFFICIAL 10개 (버퍼 12개)
# 센티멘트: negative 1 / positive 1 / free 8
# 소스: .go.kr 도메인 (국회, 정부, 공공기관)
# 기간: 최근 4년
...
Naver Search API를 사용하여 {politician_full}의 {category_kr} 관련 OFFICIAL 데이터를 수집하세요.
...
## 검색 대상
- **.go.kr 도메인** (국회, 정부, 공공기관 공식 자료)
  - assembly.go.kr (국회)
  - korea.kr (정부24)
  - moleg.go.kr (법제처)
  - mois.go.kr (행정안전부)
  - 기타 정부/공공기관 .go.kr 도메인
- 법안 발의, 국정감사 질의, 위원회 활동 기록
- 정부/국회 공식 성명, 기자회견, 정책 발표
- 공공기관 공식 보도자료, 활동 보고서

## 수집 금지
- PUBLIC 소스 (뉴스, 블로그, 커뮤니티 등)
- 의견/평가/분석 중심 콘텐츠
- 동명이인, 다른 정치인의 자료

## 센티멘트 배분 (정확한 수량 필수)
총 {remaining}개 수집 시 다음 배분을 따르세요:
- **negative 1개**: 논란, 비판, 실책, 반대 여론이 포함된 공식 기록
- **positive 1개**: 성과, 수상, 긍정 평가가 포함된 공식 기록
- **free 8개**: 센티멘트 제한 없이 자유롭게 수집 (사실 중심, 중립적 기록 포함)
...
- **webkr** 검색 API 사용 (한국 웹 검색)
- **site:.go.kr** 필터 활용하여 OFFICIAL 소스만 검색
- **sort=date** 옵션으로 최신 자료 우선
- **display=100** 으로 충분한 결과 확보
- API 응답의 **title**, **link**, **description**, **postdate** 필드 활용

## 최종 확인 사항
- [ ] {remaining}개 수집 (negative 1개 + positive 1개 + free 8개)
