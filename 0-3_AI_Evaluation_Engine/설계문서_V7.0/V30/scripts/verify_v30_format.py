...
def verify_file(file_path):
    ...
    try:
        ...
        for i, ev in enumerate(evaluations, 1):
            ...
            # V30 형식 위반 필드 확인
            if 'id' in ev:
                warnings.append(f"[항목 {i}] V30 형식 위반: 'id' 대신 'collected_data_id' 사용해야 함")
            if 'reason' in ev:
                warnings.append(f"[항목 {i}] V30 형식 위반: 'reason' 대신 'reasoning' 사용해야 함")
