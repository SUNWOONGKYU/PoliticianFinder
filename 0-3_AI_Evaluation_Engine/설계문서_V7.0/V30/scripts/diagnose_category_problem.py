...
# -*- coding: utf-8 -*-
"""
V30 카테고리 분포 문제 진단 스크립트

문제: 일부 카테고리는 229개, 일부는 3개로 불균형
원인: 전역 600개 제한이 없어서 카테고리별로 독립적으로 수집
"""
...
def diagnose_problem():
    ...
    print(f"{'='*80}")
    print(f"V30 카테고리 분포 문제 진단: {politician_name} ({politician_id})")
    print(f"{'='*80}\n")
    ...
    for cat_name, cat_korean in CATEGORIES:
        ...
        # V30 목표 계산
        expected_gemini = 90  # 50 official + 40 public
        expected_grok = 10    # 10 public
        expected_total = 100
        ...
        print(f"   AI 분포: Gemini {ai_dist.get('Gemini', 0)}개 (목표 90), Grok {ai_dist.get('Grok', 0)}개 (목표 10)")
        print(f"   타입 분포: OFFICIAL {type_dist.get('official', 0)}개 (목표 50), PUBLIC {type_dist.get('public', 0)}개 (목표 50)")
        ...
        for ai_name in ["Gemini", "Grok"]:
            for data_type in ["official", "public"]:
                for sentiment in ["negative", "positive", "free"]:
                    ...
                    # V30 목표
                    target = V30_TARGETS.get(ai_name, {}).get(data_type, {}).get(sentiment, 0)
    ...
    print("🔍 문제:")
    print("   collect_v30.py 스크립트는 카테고리별로 독립적으로 수집합니다.")
    print("   전역 600개 제한이 없어서 일부 카테고리가 과도하게 수집되었습니다.")
    print()
    ...
    print("   4. collect_v30.py에 전역 600개 제한 추가")
    print()
    ...
    print("   • Gemini 900개, Grok 100개 확인")
    print()
