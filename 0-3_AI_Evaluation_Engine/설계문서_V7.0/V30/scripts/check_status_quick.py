#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
조은희 V30 수집 현황 확인
"""

import os
import sys
from pathlib import Path
from supabase import create_client
from dotenv import load_dotenv

# UTF-8 출력 설정
if sys.platform == 'win32':
    import codecs
    sys.stdout = codecs.getwriter('utf-8')(sys.stdout.buffer, 'strict')
    sys.stderr = codecs.getwriter('utf-8')(sys.stderr.buffer, 'strict')

# .env 로드
env_path = Path(__file__).parent.parent.parent.parent / '.env'
load_dotenv(env_path)

# Supabase 초기화
supabase = create_client(
    os.getenv('SUPABASE_URL'),
    os.getenv('SUPABASE_SERVICE_ROLE_KEY')
)

politician_id = 'd0a5d6e1'

# 전체 개수
result = supabase.table('collected_data_v30') \
    .select('id', count='exact') \
    .eq('politician_id', politician_id) \
    .execute()
total = result.count if result.count else 0

# AI별 개수
result = supabase.table('collected_data_v30') \
    .select('collector_ai') \
    .eq('politician_id', politician_id) \
    .execute()

ai_counts = {}
for item in result.data:
    ai = item['collector_ai']
    ai_counts[ai] = ai_counts.get(ai, 0) + 1

# 카테고리별 개수
result = supabase.table('collected_data_v30') \
    .select('category') \
    .eq('politician_id', politician_id) \
    .execute()

cat_counts = {}
for item in result.data:
    cat = item['category']
    cat_counts[cat] = cat_counts.get(cat, 0) + 1

print(f'{"="*60}')
print(f'  조은희 V30 수집 현황')
print(f'{"="*60}\n')

print(f'총 수집: {total}개 / 500개 (목표)')
print(f'진행률: {total / 5:.1f}%\n')

print(f'AI별 분포:')
# V30 500개: Gemini 300 (60%) + Perplexity 200 (40%) - 40-20-40 배분
for ai in ['Gemini', 'Perplexity']:
    count = ai_counts.get(ai, 0)
    pct = (count / total * 100) if total > 0 else 0
    target = 300 if ai == 'Gemini' else 200  # Gemini 300, Perplexity 200
    status = '✅' if count >= target else '🔄'
    print(f'  {status} {ai}: {count}개 / {target}개 ({pct:.1f}%)')

print(f'\n카테고리별 분포:')
categories = ['expertise', 'leadership', 'vision', 'integrity', 'ethics', 'accountability',
              'transparency', 'communication', 'responsiveness', 'publicinterest']
for cat in categories:
    count = cat_counts.get(cat, 0)
    status = '✅' if count >= 60 else '🔄'
    print(f'  {status} {cat}: {count}개 / 60개')

print(f'\n{"="*60}')
