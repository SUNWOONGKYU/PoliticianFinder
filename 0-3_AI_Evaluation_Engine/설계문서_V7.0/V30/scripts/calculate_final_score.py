...
print('='*100)
print(f'V30 최종 점수 계산 - {politician_name}')
print('='*100)
print()
print('📐 V28 점수 계산 공식:')
print(f'   PRIOR = {PRIOR}')
...
for ai in ais:
    ...
    for cat_eng, cat_kor in categories:
        ...
        if result.data:
            ...
            # V28 공식: (PRIOR + avg_rating × COEFFICIENT) × 10
...
# 등급 계산
if avg_final_score >= 900:
    grade = 'S'
elif avg_final_score >= 800:
    grade = 'A'
elif avg_final_score >= 700:
    grade = 'B'
elif avg_final_score >= 600:
    grade = 'C'
elif avg_final_score >= 500:
    grade = 'D'
else:
    grade = 'F'

print(f'🏆 최종 등급: {grade}등급 ({avg_final_score}점)')
print()
print('등급 기준:')
print('  S: 900~1000점 (탁월)')
print('  A: 800~899점 (우수)')
print('  B: 700~799점 (양호)')
print('  C: 600~699점 (보통)')
print('  D: 500~599점 (미흡)')
