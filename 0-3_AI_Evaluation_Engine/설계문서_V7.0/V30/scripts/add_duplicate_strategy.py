# -*- coding: utf-8 -*-
"""
카테고리 파일에 중복 방지 전략 섹션 추가 스크립트
"""
import os
import sys
import re

# UTF-8 출력 설정
if sys.platform == 'win32':
    import io
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
    sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8')

DUPLICATE_STRATEGY_SECTION = """---

## 3. 중복 방지 전략 ✨

### 🔥 고급 중복 검사 (URL + 제목)

```
V30부터 URL + 제목 이중 검사로 강화!
```

### 3.1 URL 정규화

**파라미터, 앵커 제거로 같은 페이지 식별**

```
예시:
✅ 중복으로 감지 (같은 페이지):
- https://namu.wiki/w/김민석/비판%20및%20논란
- https://namu.wiki/w/김민석/비판%20및%20논란?rev=456
- https://namu.wiki/w/김민석/비판%20및%20논란#s-1
- https://namu.wiki/w/김민석/비판%20및%20논란?from=search

→ 모두 https://namu.wiki/w/김민석/비판%20및%20논란 로 정규화
→ 1개만 수집, 나머지는 중복으로 스킵
```

### 3.2 제목 유사도 검사

**95% 이상 유사한 제목 = 중복**

```
예시:
✅ 중복으로 감지 (95% 이상 유사):
- "김민석/비판 및 논란/국무총리 후보자 - 나무위키"
- "김민석 / 비판 및 논란 / 국무총리 후보자 - 나무위키"
- "김민석/비판 및 논란/국무총리 후보자"

→ 제목 정규화 후 비교
→ 1개만 수집, 나머지는 중복으로 스킵
```

### 3.3 중복 판단 규칙

```
┌─────────────────────────────────────────┐
│ 같은 AI가 수집한 데이터 중복 체크        │
├─────────────────────────────────────────┤
│ [조건 1] URL 정규화 후 일치             │
│    → 중복으로 판단, 스킵                │
│                                         │
│ [조건 2] 제목 95% 이상 유사             │
│    → 중복으로 판단, 스킵                │
│                                         │
│ [조건 3] 다른 AI가 같은 데이터 수집     │
│    → 중복 아님, 모두 저장 (자연 가중치) │
└─────────────────────────────────────────┘
```

### 3.4 AI에게 지시사항

**📌 수집 AI에게 (Gemini, Grok):**

```
⚠️ 중복 방지 필수:
1. 같은 소스의 다른 버전/섹션 수집 금지
   ❌ 나무위키 같은 페이지의 다른 섹션
   ❌ 뉴스 같은 기사의 다른 버전

2. 이미 수집한 제목과 95% 이상 유사하면 수집 금지
   ❌ "김민석 의혹 논란" vs "김민석 논란 의혹"
   ❌ "김민석/비판" vs "김민석 / 비판"

3. 다양한 소스에서 수집 (같은 소스 반복 금지)
   ✅ 나무위키 1개, 뉴스 1개, 블로그 1개
   ❌ 나무위키 10개 (다른 섹션)
```

---

## 4. 평가 범위 - 구체적 10개 항목"""


def process_file(filepath):
    """파일 처리: 중복 방지 전략 추가 + 섹션 번호 조정"""
    print(f"\n처리 중: {os.path.basename(filepath)}")

    # 파일 읽기
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            content = f.read()
    except Exception as e:
        print(f"  ❌ 파일 읽기 실패: {e}")
        return False

    # 이미 중복 방지 전략이 있는지 확인
    if "## 3. 중복 방지 전략" in content:
        print(f"  ⏭️ 이미 중복 방지 전략 있음, 스킵")
        return True

    # "## 3. 평가 범위" 찾기
    pattern = r'---\n\n## 3\. 평가 범위'
    match = re.search(pattern, content)

    if not match:
        print(f"  ❌ '## 3. 평가 범위' 섹션을 찾을 수 없음")
        return False

    # 중복 방지 전략 삽입
    insert_pos = match.start()
    new_content = content[:insert_pos] + DUPLICATE_STRATEGY_SECTION + content[match.end():]

    # 섹션 번호 조정 (4 → 5, 5 → 6, 6 → 7, 7 → 8, 8 → 9, 9 → 10)
    # 역순으로 치환 (9 → 10부터 시작)
    replacements = [
        (r'^## 9\. ', '## 10. '),
        (r'^## 8\. ', '## 9. '),
        (r'^## 7\. ', '## 8. '),
        (r'^## 6\. ', '## 7. '),
        (r'^## 5\. ', '## 6. '),
        (r'^## 4\. V30 기간 제한', '## 5. V30 기간 제한'),
    ]

    for pattern, replacement in replacements:
        new_content = re.sub(pattern, replacement, new_content, flags=re.MULTILINE)

    # 파일 쓰기
    try:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print(f"  ✅ 완료")
        return True
    except Exception as e:
        print(f"  ❌ 파일 쓰기 실패: {e}")
        return False


def main():
    # 카테고리 파일 목록
    categories = [
        'cat02_leadership.md',
        'cat03_vision.md',
        'cat05_ethics.md',
        'cat06_accountability.md',
        'cat07_transparency.md',
        'cat08_communication.md',
        'cat09_responsiveness.md',
        'cat10_publicinterest.md',
    ]

    base_dir = os.path.join('..', 'instructions', '2_collect')

    print("="*60)
    print("카테고리 파일 중복 방지 전략 자동 추가")
    print("="*60)

    success_count = 0
    total_count = len(categories)

    for cat_file in categories:
        filepath = os.path.join(base_dir, cat_file)
        if os.path.exists(filepath):
            if process_file(filepath):
                success_count += 1
        else:
            print(f"\n처리 중: {cat_file}")
            print(f"  ❌ 파일이 존재하지 않음: {filepath}")

    print("\n" + "="*60)
    print(f"완료: {success_count}/{total_count}개 파일 성공")
    print("="*60)


if __name__ == "__main__":
    main()
