...
negative_result = supabase.table('collected_data_v30')\
    .select('*')\
    .eq('politician_id', politician_id)\
...
print(f'{"카테고리":<20} {"Claude":>8} {"ChatGPT":>8} {"Gemini":>8} {"Grok":>8} {"합계":>8}')
print('-'*100)
...
for cat in categories:
    ...
    print(f'{cat:<20} {ai_counts.get("Claude", 0):>8} {ai_counts.get("ChatGPT", 0):>8} {ai_counts.get("Gemini", 0):>8} {ai_counts.get("Grok", 0):>8} {total:>8}')
...
if not duplicate_titles:
    print('  - 제목 완전 일치 중복 없음')
if not abnormal:
    print('  - 모든 데이터 정확히 4번씩 평가됨 (4개 AI)')
print()
