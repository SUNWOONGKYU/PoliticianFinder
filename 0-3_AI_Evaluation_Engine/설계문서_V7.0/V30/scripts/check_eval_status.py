...
print("=" * 60)
print("V30 평가 완료 현황: 김민석 (f9e00370)")
print("=" * 60)
...
for ai in ais:
    ...
    status = "✅" if count == 750 else "⚠️"
    print(f"- {ai}: {count}/750 ({percentage:.1f}%) {status}")

total = sum(total_by_ai.values())
print("=" * 60)
print(f"총 평가: {total}/3000 ({(total/3000)*100:.1f}%)")
print("=" * 60)
...
for cat_eng, cat_kor in categories:
    ...
    print(f"  {status} {cat_kor} ({cat_eng}): {count}/75")
