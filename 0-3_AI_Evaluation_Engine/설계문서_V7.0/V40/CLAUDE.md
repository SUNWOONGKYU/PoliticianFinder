...
#### 🚨 강화 규칙: Give-Up 적용 전 "오류 아님" 증명 필수

**배경**: 단순히 "4라운드 실패"만으로 포기하면, 규칙을 핑계로 수집 회피 가능

**강화된 규칙**: Give-Up을 적용하기 전에 다음 5가지 증거로 "이것이 오류가 아니다"를 증명해야만 포기 가능

```
[오류 아님 증명 체크리스트]

1️⃣ API 응답 상태: 200 OK
   ✅ 429 (Rate Limit), 500 (서버 오류) 아님
   ✅ 4라운드 모두 API 호출 성공
...
**적용 규칙**:
```python
# adjust_v40_data.py의 _verify_not_error() 함수에서 자동 검증

if all([
    api_status == 'OK',           # 1️⃣ API 정상
    search_result > 0,            # 2️⃣ 검색결과 존재
    period_data == 0,             # 3️⃣ 기간내 데이터 없음
    filter_normal == True         # 4️⃣ 필터 정상
]):
    # 모든 증거 확인 → 5️⃣ 논리적 결론 성립
    apply_leverage_score_0()  # Give-Up 적용 가능
else:
    # 증거 미확인 → 오류 의심
    raise Exception("추가 조사 필요")
```

**예시 (오준환)**:
```
증명 1: API 정상 ✅
  - 초기 429 에러 → 시간 경과 후 200 OK
  - 4라운드 모두 성공
