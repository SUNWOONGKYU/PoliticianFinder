...
for cat_eng, cat_kor in CATEGORIES:
    ...
    evaluated = supabase.table('evaluations_v40').select('id', count='exact').eq(
        'politician_id', POLITICIAN_ID
    ).eq('category', cat_eng).eq('evaluator_ai', 'ChatGPT').execute()
...
if incomplete:
    ...
    for cat_eng, cat_kor, remaining in incomplete:
        print(f"python codex_eval_helper.py --politician_id={POLITICIAN_ID} --politician_name={POLITICIAN_NAME} --category={cat_eng}")
