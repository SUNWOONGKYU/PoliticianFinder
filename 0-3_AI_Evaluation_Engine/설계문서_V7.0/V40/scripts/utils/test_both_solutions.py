...
이 스크립트는 V30→V40 전환 과정에서 API 기반 해결책을 테스트하기 위해 작성되었으며,
V40 실제 워크플로우에서는 사용되지 않습니다.
...
def main():
    ...
    if naver_result:
        print(f"\n🔶 Naver:")
        print(f"  원본: {naver_result['total']}개")
        print(f"  검증 통과: {naver_result['verified']}개")
        print(f"  URL 품질: {naver_result['success_rate']:.1f}%")
    ...
    if gemini_result and naver_result:
        if gemini_result['success_rate'] >= 80 and naver_result['success_rate'] >= 80:
            print("✅ 두 AI 모두 80% 이상 달성!")
            print("   → collect_v40.py에 적용 가능")
        elif gemini_result['success_rate'] >= 80:
            print("✅ Gemini만 80% 이상 달성")
            print("   → Gemini 방식 우선 적용")
        elif naver_result['success_rate'] >= 80:
    ...
    print("2. 해당 방식을 collect_v40.py에 적용")
