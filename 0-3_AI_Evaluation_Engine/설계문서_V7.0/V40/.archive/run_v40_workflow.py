# -*- coding: utf-8 -*-
"""
V40 전체 워크플로우 자동 실행 스크립트

프로세스:
1. 수집 (collect_v40.py)
2. 수집 검증 (품질 기준: 버퍼 20%, 최대 120% 이내)
3. 데이터 검증, 중복 제거 및 재수집 (validate_v40_fixed.py) ✅ 자동화
   - URL 실제 존재 여부 확인
...
# 스크립트 경로
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))       # V40/scripts/workflow/
CORE_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), 'core')  # V40/scripts/core/
ENGINE_ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(SCRIPT_DIR))))  # 0-3_AI_Evaluation_Engine/
...
def run_command(cmd, description, cwd=None):
    ...
    result = subprocess.run(cmd, shell=True, capture_output=False, text=True, env=env, cwd=cwd or CORE_DIR)
...
def check_collection_quality(politician_id):
    ...
    # 수집 데이터 개수 확인
    result = supabase.table('collected_data_v40').select('id', count='exact').eq('politician_id', politician_id).execute()
    ...
    print(f"수집 비율: {collect_pct:.1f}% (목표: 120% 이내, 버퍼 20%)")

    if collect_pct > 120:
        print(f"⚠️ 경고: 수집 비율이 120%를 초과했습니다 (버퍼 20% 초과).")
        print(f"  → 데이터 품질에 영향을 줄 수 있습니다.\n")
        return False

    if collected_count < 60:
        print(f"❌ 오류: 수집 데이터가 너무 적습니다 (최소 60개 필요)")
        return False
...
def check_evaluation_quality(politician_id):
    ...
    # 평가 데이터 개수
    eval_result = supabase.table('evaluations_v40').select('id', count='exact').eq('politician_id', politician_id).execute()
...
def get_missing_evaluations(politician_id):
    ...
    offset = 0
    while True:
        result = supabase.table('collected_data_v40').select('*').eq('politician_id', politician_id).range(offset, offset + 999).execute()
        if not result.data:
            break
    ...
    offset = 0
    while True:
        result = supabase.table('evaluations_v40').select('collected_data_id, evaluator_ai').eq('politician_id', politician_id).range(offset, offset + 999).execute()
        if not result.data:
            break
...
def reevaluate_missing(politician_id, politician_name, parallel=False):
    """누락된 평가 재실행 (evaluate_v40.py 재실행으로 자동 처리)"""
    ...
    print(f"재평가를 시작합니다...\n")
    print(f"▶ evaluate_v40.py를 재실행하여 누락된 평가만 처리합니다.\n")

    # evaluate_v40.py 재실행 (check_already_evaluated가 100% 체크하므로 누락된 것만 평가)
    ...
    success = run_command(cmd, "evaluate_v40.py 재실행 (누락 평가 자동 처리)")
...
def main():
    parser = argparse.ArgumentParser(description='V40 전체 워크플로우 자동 실행')
    ...
    if not args.skip_collect:
        ...
        if not run_command(cmd, "데이터 수집 실행", cwd=ENGINE_ROOT):
    ...
    # 3. 데이터 검증, 중복 제거 및 재수집 (validate_v40_fixed.py)
    print_step("3", "데이터 검증, 중복 제거 및 재수집")
    cmd = f'python validate_v40_fixed.py --politician_id={args.politician_id} --politician_name="{args.politician_name}" --no-dry-run'
    ...
    if not args.skip_evaluate:
        print_step("4", "데이터 평가")
        parallel_flag = "--parallel" if args.parallel else ""
        # 4개 AI × 10개 카테고리 개별 평가 (evaluate_v40.py는 존재하지 않음)
        ...
        for cat in CATEGORIES:
            print(f"\n  ▶ {cat} 평가 중...")
            ai_cmds = [
                (f'python {HELPERS_DIR}/claude_eval_helper.py --politician_id={args.politician_id} --politician_name="{args.politician_name}" --category={cat}', "Claude"),
                (f'python {HELPERS_DIR}/codex_eval_helper.py --politician_id={args.politician_id} --politician_name="{args.politician_name}" --category={cat}', "ChatGPT"),
                (f'python {SCRIPT_DIR}/evaluate_gemini_subprocess.py --politician="{args.politician_name}" --category={cat}', "Gemini"),
                (f'python {HELPERS_DIR}/grok_eval_helper.py --politician_id={args.politician_id} --politician_name="{args.politician_name}" --category={cat}', "Grok"),
            ]
            for cmd, ai_name in ai_cmds:
                if not run_command(cmd, f"{ai_name} {cat} 평가"):
                    print(f"  ⚠️ {ai_name} {cat} 평가 실패 (계속 진행)")
                    eval_failed = True
