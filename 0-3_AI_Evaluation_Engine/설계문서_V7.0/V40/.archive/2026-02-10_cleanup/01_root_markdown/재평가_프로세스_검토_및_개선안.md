# V40 재평가 프로세스 검토 및 개선안

**작성일**: 2026-01-25
**대상**: 1,000개 데이터 × 4개 AI = 4,000개 평가
**현재 버전**: run_v40_workflow.py + evaluate_v40.py
...
5. [Claude CLI Direct 재평가](#5-claude-cli-direct-재평가)
...
단계 1: 수집 (collect_v40.py)
├─ 4개 AI × 10개 카테고리 × 100개 = 4,000개 수집 목표
└─ 실제: ~1,000개 (중복 제거 후)

단계 2: 수집 품질 검증 (check_collection_quality)
├─ 수집 개수 확인: SELECT COUNT(*) FROM collected_data_v40
├─ 품질 기준: 버퍼 20%, 최대 120% 이내 (1,200개 이하)
└─ 최소 기준: 100개 이상

단계 3: 데이터 검증 및 재수집 (validate_v40_fixed.py)
├─ URL 존재 여부 확인
├─ 도메인 유효성 검사
├─ 기간 제한 검증
├─ 중복 제거 (같은 AI + 같은 URL)
...
단계 4: 평가 (evaluate_v40.py)
├─ 4개 AI가 각각 1,000개 전체 평가
├─ Claude: CLI Direct (수동 배치)
├─ ChatGPT/Gemini/Grok: API 자동 평가
└─ 결과: evaluations_v40 테이블에 저장
...
단계 7: 점수 계산 (calculate_v40_scores.py)
└─ 카테고리별/최종 점수 계산
```
...
- 네트워크 왕복: 2번 (collected_data_v40, evaluations_v40)
- **예상 소요 시간**: ~1초
...
def reevaluate_missing(politician_id, politician_name, parallel=False):
    ...
    success = run_command(cmd, "evaluate_v40.py 재실행")
...
2. `evaluate_v40.py` 재실행 → 전체 평가 다시 체크 (~10분)
3. `get_missing_evaluations` 재호출 → 0개 확인 (~5초)

**문제점**:
- ❌ **중복 작업**: evaluate_v40.py가 내부적으로 check_already_evaluated 호출
  - reevaluate_missing이 이미 누락을 찾았는데
  - evaluate_v40.py가 다시 전체 체크
...
evaluate_v40.py 재실행      # 내부에서 check_already_evaluated × 40회: 10초
...
**영향**:
- 재평가 시작 전 5초 대기
- 재평가 중 10초 체크 (evaluate_v40.py)
- 재평가 후 5초 재확인
- **총 20초의 불필요한 오버헤드**

#### 문제 3: evaluate_v40.py 전체 재실행

**현상**:
```python
# 120개만 누락되어도 전체 프로세스 실행
cmd = f'python evaluate_v40.py --politician_id={id} --politician_name="{name}"'
# evaluate_v40.py가 10개 카테고리 × 4개 AI = 40번 check_already_evaluated 실행
```
...
**현재 로직** (evaluate_v40.py Line 627-666):
```python
...
| evaluate_v40.py 재실행 | 10분 | 100분 | 10× (문제) |
...
10:00 evaluate_v40.py 재실행 (10분) ❌ 느림
...
if needs_reevaluation:
    reevaluate_missing()  # evaluate_v40.py 재실행
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='V40 누락 평가 조회 (최적화)')
...
def reevaluate_missing_improved(politician_id, politician_name, parallel=False):
    ...
    2. ChatGPT/Gemini/Grok: evaluate_v40.py 재실행
    3. 상세 진행 상황 표시
    """
    ...
    # 4. ChatGPT/Gemini/Grok 재평가 (evaluate_v40.py)
    if other_missing:
        print(f"\n{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(description='V40 전체 워크플로우 자동 실행 (개선)')
    parser.add_argument('--politician_id', required=True, help='정치인 ID')
    parser.add_argument('--politician_name', required=True, help='정치인 이름')
    ...
    print(f"\n{'#'*60}")
    print(f"# V40 전체 워크플로우 자동 실행 (개선 버전)")
...
1. **run_v40_workflow_improved.py** 전체 배포
   - 기존 워크플로우 백업
   - 개선 버전 배포
   - 전체 테스트 (3명 정치인)
