...
|----|----------|---------|-----|--------|----------|
| **Claude** | ❌ 불가 (CLI Direct) | - | - | - | 불필요 |
| **ChatGPT** | ✅ Automatic | 1024 | 5분~24시간 | 50% | 불필요 |
| **Gemini** | ❌ 불가 (CLI 방식) | - | - | - | 불필요 |
...
### 4.4 Gemini (CLI 방식 - API 캐싱 해당 없음)

> **⚠️ V40에서 Gemini는 CLI 방식을 사용합니다.**
> API가 아닌 Gemini CLI 터미널에서 직접 실행하므로, API 캐싱 전략은 적용되지 않습니다.
>
> **Gemini CLI 평가 프로세스:**
> 1. `gemini_eval_helper.py fetch` → 평가 데이터 준비
> 2. Gemini CLI 터미널에 프롬프트 붙여넣기 → 결과 받기
> 3. `gemini_eval_helper.py save` → 결과 DB 저장
>
> CLI 방식이므로 비용은 $0 (무료)이며, API 캐싱 최적화가 불필요합니다.
...
**Gemini (CLI 방식 - 단일 프롬프트)**:
```python
# gemini_eval_helper.py에서 프롬프트 생성 → CLI 터미널에 붙여넣기
...
if ai_name == "ChatGPT":
    ...
    return call_gemini_cli_evaluation(...)  # CLI 방식 (gemini_eval_helper.py)
elif ai_name == "Grok":
...
|----|----------|-----------|--------|---------|
| **Claude** | 불가 (CLI Direct) | - | 0% | 배치 50개 + 병렬 세션 |
| **ChatGPT** | 자동 + Extended | 낮음 ✅ | **75-81%** | Extended Caching 적용 |
| **Gemini** | 불가 (CLI 방식) | - | 0% | CLI 터미널에서 직접 실행 |
