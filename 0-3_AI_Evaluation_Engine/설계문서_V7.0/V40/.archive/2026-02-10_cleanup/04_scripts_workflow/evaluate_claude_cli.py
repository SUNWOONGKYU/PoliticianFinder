#!/usr/bin/env python3
"""
Claude CLI 직접 실행 - AI 평가
===============================

Claude CLI를 subprocess로 직접 실행하여 데이터를 평가합니다.

사용법:
    python evaluate_claude_cli.py --politician-id 8c5dcc89 --politician-name "박주민"
"""
...
def run_claude_cli(prompt: str, timeout: int = 180) -> Dict:
    """Claude CLI 직접 실행"""
    ...
    try:
        ...
        return {
            'success': False,
            'error': f"Claude CLI timeout ({timeout}s)"
        }
...
def evaluate_claude_parallel(politician_id: str, politician_name: str,
    ...
    print(f"\n{'='*60}")
    print(f"🤖 Claude CLI 병렬 평가 시작 - {politician_name}")
    ...
    print(f"\n{'='*60}")
    print(f"✅ Claude 평가 완료 - {elapsed:.1f}초")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Claude CLI 자동 평가')
