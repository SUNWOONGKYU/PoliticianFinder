#!/usr/bin/env python3
"""
Gemini MCP 클라이언트 - 데이터 수집
====================================

[OK] Gemini CLI를 MCP를 통해 사용 (비용 절감!)
...
사용법:
    python collect_gemini_mcp.py --politician-id 8c5dcc89 --politician-name "박주민"
"""
    ...
    for sentiment in ['negative', 'positive', 'free']:
        ...
        try:
            ...
            if result_dict.get('success'):
                ...
                error_msg = f"OFFICIAL/{sentiment}: {result_dict.get('error', 'Unknown')}"
    ...
    for sentiment in ['negative', 'positive', 'free']:
        ...
        try:
            ...
            if result_dict.get('success'):
                ...
                error_msg = f"PUBLIC/{sentiment}: {result_dict.get('error', 'Unknown')}"
                errors.append(error_msg)
                print(f"  [ERROR] {error_msg}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Gemini MCP 자동 수집')
