...
OpenAI API를 사용하여 데이터를 평가합니다.

사용법:
    python evaluate_chatgpt_api.py --politician-id 8c5dcc89 --politician-name "박주민"
"""
...
def call_chatgpt_api(prompt: str) -> Dict:
    """ChatGPT API 호출"""
        ...
        try:
            ...
            if result.data:
                supabase.table('collected_data_v40') \
                    .update({'evaluated_by_chatgpt': True}) \
                    .eq('id', data_id) \
                    .execute()
...
def evaluate_single_category(politician_id: str, politician_name: str,
    ...
    print(f"🤖 [{category}] ChatGPT 평가 시작...")
...
def evaluate_chatgpt_parallel(politician_id: str, politician_name: str,
                             max_workers: int = 10,
                             batch_size: int = 25) -> Dict:
    """10개 카테고리 병렬 ChatGPT 평가"""

    print(f"\n{'='*60}")
    print(f"🤖 ChatGPT API 병렬 평가 시작 - {politician_name}")
    ...
    print(f"\n{'='*60}")
    print(f"✅ ChatGPT 평가 완료 - {elapsed:.1f}초")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ChatGPT API 자동 평가')
    ...
    result = evaluate_chatgpt_parallel(
        args.politician_id,
