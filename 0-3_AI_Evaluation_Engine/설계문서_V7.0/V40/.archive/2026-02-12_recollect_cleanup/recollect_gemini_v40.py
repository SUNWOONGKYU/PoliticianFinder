...
사용법:
  # 현재 상태 확인만
  python recollect_gemini_v40.py --politician_id=ID --politician_name="이름"

  # 부족분 추가 수집
  python recollect_gemini_v40.py --politician_id=ID --politician_name="이름" --mode=add

  # 특정 카테고리만
  python recollect_gemini_v40.py --politician_id=ID --politician_name="이름" --category=expertise --mode=add

  # 전체 교체 (삭제 후 재수집)
  python recollect_gemini_v40.py --politician_id=ID --politician_name="이름" --mode=replace
"""
...
def analyze_current_status(politician_id: str, category: str = None) -> Dict:
    ...
    for cat in categories_to_check:
        ...
        # 부족분 계산 (100개 목표 기준)
        official_shortage = max(0, GEMINI_TARGETS['official'] - official_count)
        public_shortage = max(0, GEMINI_TARGETS['public'] - public_count)
        ...
        print(f"  OFFICIAL: {official_count:3d}개 (목표: 30개, 부족: {official_shortage}개)")
        print(f"  PUBLIC:   {public_count:3d}개 (목표: 20개, 부족: {public_shortage}개)")
...
def delete_existing_data(politician_id: str, category: str):
    """기존 Gemini 데이터 삭제 (replace 모드용)"""
    print(f"\n⚠️  [{category}] 기존 Gemini 데이터 삭제 중...")
...
def recollect_category(politician_id: str, politician_name: str, category: str):
    ...
    """
    print(f"\n{'='*80}")
    print(f"[{category}] Gemini 재수집 시작")
    print(f"{'='*80}")
    ...
    collect_script = SCRIPT_DIR / "collect_gemini_subprocess.py"
...
def main():
    parser = argparse.ArgumentParser(description='V40 Gemini 재수집 스크립트')
    ...
    if args.mode == 'verify':
        ...
        if need_recollect > 0:
            ...
            print(f"   python recollect_gemini_v40.py --politician_id={args.politician_id} \\")
