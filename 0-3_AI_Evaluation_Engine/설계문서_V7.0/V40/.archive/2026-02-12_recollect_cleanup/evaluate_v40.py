...
=== 핵심 원칙 ===
- 풀링 방식: 4개 AI 수집 데이터를 합쳐서 평가
- 등급 체계: V40 8등급 (+4 ~ -4, 0 없음, X=제외, 점수 = 등급 × 2)
- 배치 평가: 25개씩 배치 평가
- 정치인 프로필: format_politician_profile
- ✨ Claude CLI: API 대신 Claude Code CLI 사용 (CLI Direct)

=== V40 풀링 평가 프로세스 ===
[1] 풀링: 수집 채널 (Gemini 50 + Naver 50) 데이터 통합 (카테고리별 최대 100개, 버퍼 20% 포함 120개)
    - 중복 제거: 같은 AI가 같은 URL 2번 수집 → 1개만 유지
              다른 AI가 같은 URL 수집 → 모두 유지
[2] 평가: Claude(CLI), Gemini(CLI), ChatGPT(API), Grok(API) 각각 전체 데이터 평가
    - ✨ Claude: CLI Direct 호출 (claude_eval_helper.py)
    - ✨ Gemini: CLI 호출 (gemini_eval_helper.py)
    - ChatGPT, Grok: API 호출
...
AI_CONFIGS = {
    ...
    "Grok": {
        "model": "grok-3-mini",  # 경량 모델 (Haiku 4.5급)
        "env_key": "XAI_API_KEY",
        "base_url": "https://api.x.ai/v1"
    },
    "Gemini": {
        "model": "gemini-2.5-flash",  # CLI에서 사용하는 모델
        "env_key": None  # ⚠️ Gemini는 CLI 사용 (API 키 불필요)
    }
}

# V40 등급 체계 (+4 ~ -4, X=제외) - 8등급
VALID_RATINGS = ['+4', '+3', '+2', '+1', '-1', '-2', '-3', '-4', 'X']

# ⚠️ 점수 계산은 calculate_v40_scores.py의 단독 책임
# evaluate_v40.py는 rating(등급)만 저장, score는 저장하지 않음
...
def call_claude_direct_evaluation(prompt):
    """Claude CLI Direct Mode - evaluate_v40.py에서 직접 호출 불가.
    ...
    print("\n" + "="*60)
    print("⚠️  Claude는 CLI Direct Mode로만 평가 가능합니다.")
    print("="*60)
    print("\n📋 Claude Code 명령어:")
    ...
    print("\n💡 Gemini도 CLI 방식입니다:")
    print("  python gemini_eval_helper.py fetch → Gemini CLI에서 평가 → python gemini_eval_helper.py save")
    print("="*60 + "\n")
...
def call_gemini_cli_evaluation(prompt):
    """Gemini CLI Subprocess - evaluate_v40.py에서 직접 호출 불가.

    Gemini 평가는 Gemini CLI Subprocess 방식으로 실행하세요:
    cd scripts/workflow
    python evaluate_gemini_subprocess.py --politician "{이름}" --category "{카테고리}"
    """
    import json

    print("\n" + "="*60)
    print("⚠️  Gemini는 evaluate_gemini_subprocess.py로 평가하세요.")
    print("="*60)
    print("\n📋 Gemini CLI Subprocess 평가:")
    print("  cd scripts/workflow")
    print("  python evaluate_gemini_subprocess.py --politician \"{이름}\" --category \"{카테고리}\"")
    print("\n💡 또는 API 기반 AI만 먼저 실행:")
    print("  python evaluate_v40.py --politician_id=ID --politician_name=이름 --ai=ChatGPT")
    print("  python evaluate_v40.py --politician_id=ID --politician_name=이름 --ai=Grok")
    print("="*60 + "\n")

    # 빈 결과 반환 (Gemini subprocess에서 별도 처리)
    return json.dumps({'evaluations': []}, ensure_ascii=False)
...
def call_ai_api(ai_name, system_prompt, user_prompt):
    ...
    # Claude는 CLI Direct Mode (수동)
    if ai_name == "Claude":
        return call_claude_direct_evaluation(f"{system_prompt}\n\n{user_prompt}")

    # Gemini는 CLI Subprocess 방식 (evaluate_gemini_subprocess.py 사용)
    if ai_name == "Gemini":
        return call_gemini_cli_evaluation(f"{system_prompt}\n\n{user_prompt}")

    # ChatGPT, Grok만 API 사용
...
def get_category_instructions(category_name):
    """카테고리별 평가 instruction 로드

    instructions/3_evaluate/cat01_expertise.md 등의 파일에서
    평가 기준을 읽어옵니다.

    Args:
        category_name: 카테고리 영문명 (expertise, leadership 등)

    Returns:
        평가 instruction 텍스트 또는 None
    """
    # 카테고리 이름 → 파일명 매핑
    category_files = {
        'expertise': 'cat01_expertise',
        'leadership': 'cat02_leadership',
        'vision': 'cat03_vision',
        'integrity': 'cat04_integrity',
        'ethics': 'cat05_ethics',
        'accountability': 'cat06_accountability',
        'transparency': 'cat07_transparency',
        'communication': 'cat08_communication',
        'responsiveness': 'cat09_responsiveness',
        'publicinterest': 'cat10_publicinterest'
    }

    cat_file = category_files.get(category_name.lower())
    if not cat_file:
        return None

    script_dir = os.path.dirname(os.path.abspath(__file__))
    instruction_path = os.path.join(
        script_dir, '..', 'instructions', '3_evaluate', f'{cat_file}.md'
    )
    ...
    try:
        if os.path.exists(instruction_path):
            ...
            # "## 3. 평가 범위" 섹션 추출 (핵심 평가 기준)
            import re
            match = re.search(r'## 3\. 평가 범위.*?(?=\n## |\Z)', content, re.DOTALL)
            if match:
                return match.group(0).strip()

            # 전체 내용 반환 (섹션 추출 실패 시)
            return content[:1000]  # 첫 1000자만
    except Exception as e:
...
def evaluate_batch(evaluator_ai, items, category_name, politician_id, politician_name):
    """배치 평가 (V40: 최대 25개씩, system/user 프롬프트 분리)"""
    ...
    # 카테고리별 평가 기준 로드
    category_guide = get_category_instructions(category_name)
...
총 {len(items)}개 항목 전부 평가 필수 (빠짐없이). 유효→+4~-4 | 제외대상→X"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 유효성 검증 및 ID 매칭
            # ⚠️ AI 응답 ID 우선 사용, 순서 매칭은 fallback (AI가 순서 변경 가능)
            item_ids = {str(item.get('id', '')): item.get('id') for item in items if item.get('id')}
            ...
            for idx, ev in enumerate(evaluations):
                ...
                if rating in VALID_RATINGS:
                    ev['rating'] = rating
                    # ⚠️ score는 저장하지 않음 — calculate_v40_scores.py에서 계산

                    # ID 매칭: AI 응답 ID가 유효하면 우선 사용, 아니면 순서 매칭
                    ai_returned_id = str(ev.get('id', '')).strip()
                    if ai_returned_id and ai_returned_id in item_ids:
                        ev['id'] = item_ids[ai_returned_id]
                    elif idx < len(items):
...
def check_already_evaluated(politician_id, evaluator_ai, category_name):
    ...
    try:
        ...
        # 평가된 개수 >= 수집된 개수면 완료 (중복 존재 시에도 안전)
        ...
        if not is_complete and evaluated_count > 0:
            print(f"    ⚠️ 부분 완료 발견: {evaluated_count}/{collected_count} ({completion_rate*100:.1f}%) - 미평가 항목만 추가 평가")
...
def evaluate_category(evaluator_ai, politician_id, politician_name, category_name, category_korean):
    ...
    # ✅ 이미 평가된 collected_data_id 제외 (부분 완료 시 중복 방지)
    # claude_eval_helper.py / gemini_eval_helper.py와 동일한 방식
    try:
        ...
        if evaluated_ids:
            before = len(items)
            items = [item for item in items if item.get('id') not in evaluated_ids]
            skipped = before - len(items)
            if skipped > 0:
                print(f"    ⏭️ 이미 평가된 {skipped}개 제외, 미평가 {len(items)}개 대상")
    except Exception as e:
        print(f"    ⚠️ 기평가 필터링 실패 (전체 평가 진행): {e}")

    if not items:
        print(f"    ⏭️ 모든 데이터 이미 평가 완료")
        return 0

    print(f"    📊 미평가 데이터 {len(items)}개 로드")

    # 배치 평가 (V40: 25개씩)
    total_evaluated = 0
    batch_size = 25  # V40 최적화: 25개 배치 (가이드 권장)
...
def evaluate_all(politician_id, politician_name, target_ai=None, target_category=None, parallel=False):
    ...
    # Claude, Gemini는 CLI 전용 - API 루프에서 제외
    CLI_ONLY_AIS = ["Claude", "Gemini"]
    api_ais = [ai for ai in eval_ais if ai not in CLI_ONLY_AIS]
    cli_ais = [ai for ai in eval_ais if ai in CLI_ONLY_AIS]

    if "Claude" in cli_ais:
        print(f"\n  [Claude] ⏭️ CLI Direct Mode 전용 - /evaluate-politician-v40 명령으로 별도 실행")
    if "Gemini" in cli_ais:
        print(f"\n  [Gemini] ⏭️ CLI Mode 전용 - gemini_eval_helper.py fetch → Gemini CLI 평가 → save")

    if not api_ais:
        print(f"\n⚠️  선택된 AI가 모두 CLI 전용입니다.")
        print(f"  Claude: /evaluate-politician-v40 --politician_id={politician_id} --politician_name={politician_name} --category=all")
        print(f"  Gemini: python gemini_eval_helper.py fetch → Gemini CLI에서 평가 → python gemini_eval_helper.py save")
        return 0
...
def main():
    parser = argparse.ArgumentParser(description='V40 풀링 평가')
