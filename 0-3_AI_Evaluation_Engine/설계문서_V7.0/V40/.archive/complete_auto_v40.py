...
- Gemini 2.0 Flash (CLI Subprocess, 배치 25)
- Grok 3 (xAI API Direct, 배치 25)

최적화 전략:
1. 배치 처리 (API 25개, Gemini Subprocess 25개)
...
def phase2_validation(politician_id: str, politician_name: str, tracker: ProgressTracker) -> bool:
    """Phase 2: 수집 데이터 검증"""
...
def phase5_report(politician_id: str, politician_name: str,
    ...
    try:
        from generate_report_v40 import generate_report_v40

        # 보고서 생성
        report_path = generate_report_v40(politician_id, politician_name)
...
""")
    ...
    # Phase 2: 검증
    if not args.skip_collection:
        if not phase2_validation(args.politician_id, args.politician_name, tracker):
