...
> **참고**: Gemini는 수집/평가 모두 CLI 터미널에서 직접 실행합니다.
> - 수집: `gemini_collect_helper.py fetch` → Gemini CLI에 프롬프트 붙여넣기 → `gemini_collect_helper.py save`
> - 평가: `gemini_eval_helper.py fetch` → Gemini CLI에 프롬프트 붙여넣기 → `gemini_eval_helper.py save`
> - **에러 처리**: CLI Direct 에러 처리 방법은 → 4.2.1절 참조
...
│                     ↓                                       │
├─────────────────────────────────────────────────────────────┤
│ [Phase 2] 수집 - collect_v40.py (2개 채널)                  │
│                                                             │
...
│   예) 풀 1,050개 → 기대 4,200개 → 97% = 4,074개 이상       │
│ ✓ 미달 시: 누락 평가 재실행 (자동)                          │
│                     ↓                                       │
├─────────────────────────────────────────────────────────────┤
...
V40에는 두 가지 평가 방법이 있으며, **AI별로 사용하는 방법이 다릅니다.**

| AI | 방법 | 도구 | 비용 | 실행 |
|----|------|------|------|------|
| **Claude** | 방법 1 (Helper) | `helpers/claude_eval_helper.py` | $0 (CLI Direct) | 수동 (프롬프트 붙여넣기) |
| **Gemini** | 방법 1 (Helper) | `helpers/gemini_eval_helper.py` | $0 (CLI Direct) | 수동 (프롬프트 붙여넣기) |
| **ChatGPT** | 방법 2 (API) | `core/evaluate_v40.py` | API 비용 | 자동 (API 호출) |
| **Grok** | 방법 2 (API) | `core/evaluate_v40.py` | API 비용 | 자동 (API 호출) |
...
#### 방법 2: evaluate_v40.py - API 호출 (ChatGPT, Grok용)

API를 통해 자동 평가. 별도 수동 작업 불필요.
...
> **참고**: `run_v40_workflow.py`는 방법 2(API)를 자동 실행하지만,
> 방법 1(CLI Direct)은 수동으로 별도 수행해야 합니다.

### 4.2.1 CLI Direct 에러 처리

CLI Direct (Gemini, Claude) 실행 시 에러 발생 처리 절차:

**1. JSON 형식 검증**: 저장 전 반드시 JSON 유효성 확인

**2. 재시도**: 실패 시 최대 3회 재시도

**3. 부분 실패**: 일부 카테고리만 실패 시 해당 카테고리만 재수집/재평가

**4. 완전 실패**: 3회 재시도 후에도 실패 → 에러 로그 기록 + 운영자 확인 요청
