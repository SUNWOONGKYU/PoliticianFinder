...
│                                                             │
│ 2. politicians_v40 테이블에 INSERT (11개 데이터 필드)       │
│    - id (TEXT, 8자리 hex)                                   │
│    - name, party, position, identity, title                 │
...
8. **district**: 지역구 (해당 시)
9. **gender**: 성별 ('남' 또는 '여')
10. **birth_year**: 출생연도 (INTEGER)
11. **age**: 현재 나이 (INTEGER)
...
**테이블**: `politicians_v40` (평가 전용)

**⚠️ 중요**: 이 테이블은 AI 평가 전용입니다. 웹사이트 프로필용 필드(email, website, image_url)는 포함하지 않습니다.
...
- [ ] 모든 11개 필드가 채워져 있는가?
- [ ] gender가 '남' 또는 '여'인가?
- [ ] birth_year와 age가 INTEGER 타입인가?
- [ ] identity 값이 정의된 5가지 중 하나인가?
- [ ] title 값이 정의된 6가지 중 하나인가?
...
**스크립트 위치**: `scripts/core/adjust_v40_data.py` (작성 예정)
...
| **Grok** | Grok 2 | API 호출 | `helpers/grok_eval_helper.py` | 25 | API 비용 | xAI API 호출 |
...
| **비용** | Claude/Gemini: $0 (구독)<br>ChatGPT: $0.225/1K 평가<br>총계: ~$0.23/1K 평가 | Claude: $0.75/1K<br>Gemini: $0.19/1K<br>ChatGPT: $45/1K<br>총계: ~$46/1K 평가 |
| **절감률** | **99.5%** (200배 저렴) | - |
