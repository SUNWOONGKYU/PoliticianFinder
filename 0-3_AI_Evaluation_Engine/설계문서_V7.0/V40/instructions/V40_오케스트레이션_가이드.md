...
response = supabase.table('politicians_v40').select('*').eq('id', politician_id).execute()

politician_data = response.data[0]
```

**확보해야 할 정보 (11개 데이터 필드)**:
```python
{
  ...
  "birth_year": {BIRTH_YEAR},        # 출생연도 (INTEGER)
  "age": {AGE}                       # 나이 (INTEGER)
}
```
...
**⚠️ 주의**: politicians_v40 테이블은 평가 전용입니다. 웹사이트 프로필용 필드(email, website, image_url)는 일반 politicians 테이블에 있어야 하며, 평가 시에는 필요하지 않습니다.
...
**⭐ 시간 절약 팁**:
```
Phase 2에서 버퍼 목표(60개/카테고리)로 수집 → Step 2-2 거의 스킵!
Phase 2에서 최소 목표(50개)만 수집 → Step 2-2 재수집 2-3시간 소요!
```
...
💡 **핵심**: "API가 아니라 CLI로 가라. 구독 플랜이 API보다 200배 저렴하다."
...
# Gemini (2.0 Flash) 평가 - 배치 25개 + Pre-filtering
cd scripts/workflow
...
- ✅ Gemini Subprocess - 50개 배치 자동 평가
...
def step_0_gather_politician_info(politician_id: str):
    ...
    print(f"  출생연도: {politician['birth_year']}")
    print(f"  나이: {politician['age']}세")

    # 필수 정보 검증 (11개 필드)
    required_fields = [
        'id', 'name', 'party', 'position', 'identity',
        'title', 'region', 'district', 'gender', 'birth_year', 'age'
    ]
...
def step_1_collect_data(politician_id: str, politician_name: str):
    """Step 1: 데이터 수집 (1,000개, 버퍼 20% 포함 최대 1,200개)"""
    print(f"\n[Step 1] 데이터 수집 시작...")
    print(f"  배분: Gemini 50% (500개) + Naver 50% (500개)")

    # collect_v40.py 스크립트 실행
    os.system(f'python collect_v40.py --politician_id={politician_id} --politician_name="{politician_name}"')
...
def main():
    """메인 함수"""
    if len(sys.argv) < 2:
        print("사용법: python run_v40_workflow.py <politician_id>")
        sys.exit(1)
...
- `scripts/collect_v40.py` - 수집 스크립트
- `scripts/helpers/*_eval_helper.py` - AI별 평가 스크립트
- `scripts/calculate_v40_scores.py` - 점수 계산 스크립트
