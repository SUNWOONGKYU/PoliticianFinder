...
- ❌ **비권장: 100개만 수집 → Phase 2-2 재수집 필요 (시간 낭비!)**
...
```
✅ 올바른 프로세스 (권장):
초기 수집 120개 (버퍼 포함) → 검증 삭제 → 100-120개 유지 → Phase 2-2 스킵!
                                                              ^^^^^^^^
                                          시간 절약: 2-3시간 단축!

❌ 비효율 프로세스 (비권장):
초기 수집 100개 → 검증 삭제 → 90개 남음 → Phase 2-2 재수집 (2-3시간)
                                              ^^^^^^^^^^^^^^^^^^^
                                              시간 낭비!
```
...
3. **시간 절약**: Phase 2-2 재수집 2-3시간 절약
...
### Gemini (50%) - Gemini CLI Subprocess (3단계 Fallback) ✅ 무료

**방식**: Gemini CLI Subprocess

**3단계 Fallback (자동)**:
- Step 1: CLI gemini-2.5-flash (우선, Tier 1)
- Step 2: CLI gemini-2.0-flash (quota 소진 또는 timeout 시 자동 전환)
...
**평가 방식**:
- CLI Direct: Claude Code 세션에서 직접 평가 (API 아님, $0)
- Skill 자동화: `/evaluate-politician-v40` (50개 배치 자동 평가)
**배치 크기**: 25개 (기본), 50개 (Skill 자동화)
**수집 제외 사유**: web_search 도구 검색당 $0.01 추가 과금

### Gemini (2.5 Flash → 2.0 Flash → REST API) - CLI Subprocess ✅ 수집 + 평가
**평가 방식**: Gemini CLI Subprocess (`evaluate_gemini_subprocess.py`, $0)
**배치 크기**: 25개
**3단계 Fallback**: CLI 2.5-flash → CLI 2.0-flash (quota/timeout 시) → REST API (모든 CLI 실패 시)
**프로세스**: instruction 파일 로드 → CLI 실행(timeout=300s) → 실패 시 API fallback → DB 저장
...
# Step 1: 등급 → 점수 변환
RATING_TO_SCORE = {'+4': 8, '+3': 6, '+2': 4, '+1': 2,
                   '-1': -2, '-2': -4, '-3': -6, '-4': -8}
# X (제외) 항목은 avg_score 계산에서 제외

# Step 2: 카테고리 점수 계산 (20~100점)
avg_score = sum(scores) / count  # scores = 각 항목의 변환된 점수 리스트
...
**공식 검증 예시:**
- 전체 +4인 경우: avg_score=8, (6.0 + 8×0.5)×10 = **100점** (최대)
- 전체 -4인 경우: avg_score=-8, (6.0 + -8×0.5)×10 = **20점** (최소)
- 전체 0점 데이터 없는 경우 (25개 미만 포기): avg_score=0, (6.0 + 0×0.5)×10 = **60점**
...
```
✅ Claude: $0 (CLI Direct, Anthropic 구독)
✅ Codex/ChatGPT: ~$0 (Codex CLI Direct, OpenAI 구독)
✅ Gemini: $0 (CLI Subprocess, Google AI Pro 구독)
⚠️ Grok: 소량 과금 (xAI Agent Tools API, API Key 필요)

→ 수집 $0 + 평가 ≈ $0 = 총 $0/100명 (대부분 무료)
```
...
- Grok: 1,000개 (xAI Agent Tools API)
- 총 4,000개 평가
...
```bash
# Gemini 수집 (단일 카테고리, 여러 번 실행해서 60개 이상 확보)
cd scripts/workflow
python collect_gemini_subprocess.py \
  --politician "이름" --category expertise

# Naver 수집 (단일 카테고리)
cd scripts/workflow
python collect_naver_v40_final.py \
  --politician-id ID --politician-name "이름" --category expertise

# 전체 10개 카테고리 수집 (반복 실행)
...
for cat in $CATEGORIES; do
  python collect_gemini_subprocess.py --politician "이름" --category $cat
done
for cat in $CATEGORIES; do
  python collect_naver_v40_final.py --politician-id ID --politician-name "이름" --category $cat
done
```
...
# Phase 2: 데이터 검증
cd scripts/core
python validate_v40_fixed.py --politician_id=ID --politician_name="이름" --no-dry-run
```
...
# 2. Claude Code에서 직접 평가 수행 (fetch 결과 JSON 기반)
