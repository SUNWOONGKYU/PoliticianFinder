...
3. 데이터 검증, 중복 제거 및 재수집 (validate_v40.py) ✅ 자동화
   - URL 실제 존재 여부 확인
...
def main():
    ...
    # 3. 데이터 검증, 중복 제거 및 재수집 (validate_v40.py)
    print_step("3", "데이터 검증, 중복 제거 및 재수집")
    cmd = f'python validate_v40.py --politician_id={args.politician_id} --politician_name="{args.politician_name}" --mode=all'
