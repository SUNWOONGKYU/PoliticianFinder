...
사용법:
    # 1. 평가할 데이터 조회
    python gemini_eval_helper.py fetch --politician_id=d0a5d6e1 --politician_name=조은희 --category=communication

    # 2. 평가 결과 저장
    python gemini_eval_helper.py save --politician_id=d0a5d6e1 --politician_name=조은희 --category=communication --input=gemini_eval_result.json
...
CATEGORY_KR = {
    ...
    'integrity': '청렴성', 'ethics': '윤리', 'accountability': '책임감',
    'transparency': '투명성', 'communication': '소통', 'responsiveness': '대응성',
    'publicinterest': '공익'
}
...
def cmd_save(args):
    ...
    for ev in evaluations:
        try:
            ...
            # collected_data_id 추출 (개선: 평가-수집 데이터 연결)
            ...
            db_item = {
                ...
                'category': category,
                'evaluator_ai': 'Gemini',
                'collected_data_id': collected_data_id,  # ← 추가!
                'rating': rating,
                'score': score,
