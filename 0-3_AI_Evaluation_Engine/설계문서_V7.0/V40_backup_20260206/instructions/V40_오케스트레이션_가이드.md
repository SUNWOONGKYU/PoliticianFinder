...
**목적**: 2개 AI가 분담하여 총 1,000개 데이터 수집 (버퍼 20% 포함 최대 1,200개)
...
센티멘트 배분 (차등):
- OFFICIAL (40개): negative 10% (4개) / positive 10% (4개) / free 80% (32개)
- PUBLIC (60개): negative 20% (12개) / positive 20% (12개) / free 60% (36개)
- 통합 (100개): negative 16% (16개) / positive 16% (16개) / free 68% (68개)
```
...
- ✅ 2개 AI가 동일 뉴스 수집 → 풀에 2번 포함 → 자연스럽게 2배 영향

**자연 가중치 예시**:
```
뉴스 A: Gemini만 수집 → 풀에 1번 포함 → 평가 시 1배 영향
뉴스 B: 2개 AI 모두 수집 → 풀에 2번 포함 → 평가 시 2배 영향
```
...
def step_3_evaluate_data(politician_id: str, politician_name: str):
    ...
    # evaluate_v40.py 스크립트 실행
    os.system(f'python evaluate_v40.py --politician_id={politician_id} --politician_name="{politician_name}"')
...
- `scripts/evaluate_v40.py` - 평가 스크립트
...
### 수집 중
- [ ] 2개 AI 분담 확인 (Gemini 50% + Naver 50%)
