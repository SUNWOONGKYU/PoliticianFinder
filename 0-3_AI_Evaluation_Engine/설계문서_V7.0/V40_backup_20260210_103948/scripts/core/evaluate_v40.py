...
def call_gemini_cli_evaluation(prompt):
    """Gemini CLI Mode - evaluate_v40.py에서 직접 호출 불가.

    Gemini 평가는 반드시 Gemini CLI 터미널에서 실행해야 합니다:
    python gemini_eval_helper.py fetch → Gemini CLI에 프롬프트 붙여넣기 → python gemini_eval_helper.py save
    """
    import json

    print("\n" + "="*60)
    print("⚠️  Gemini는 CLI Mode로만 평가 가능합니다.")
    print("="*60)
    print("\n📋 Gemini CLI 평가 방법:")
    print("  1. python gemini_eval_helper.py fetch  (평가 데이터 준비)")
    print("  2. Gemini CLI 터미널에 프롬프트 붙여넣기")
    print("  3. python gemini_eval_helper.py save   (결과 DB 저장)")
    print("\n💡 또는 API 기반 AI만 먼저 실행:")
    print("  python evaluate_v40.py --politician_id=ID --politician_name=이름 --ai=ChatGPT")
    print("  python evaluate_v40.py --politician_id=ID --politician_name=이름 --ai=Grok")
    print("="*60 + "\n")

    # 빈 결과 반환 (Gemini CLI에서 별도 처리)
    return json.dumps({'evaluations': []}, ensure_ascii=False)
...
def call_ai_api(ai_name, system_prompt, user_prompt):
    ...
    # Gemini도 CLI 방식 (Claude와 동일하게 스크립트에서 직접 호출 불가)
    if ai_name == "Gemini":
