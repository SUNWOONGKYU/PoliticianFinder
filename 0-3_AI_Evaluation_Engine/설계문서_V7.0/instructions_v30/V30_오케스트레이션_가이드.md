# V30 오케스트레이션 가이드

**작성일**: 2026-01-17
**버전**: V30
**대상**: Claude Code 메인 에이전트
**목적**: V30 정치인 평가 시스템의 전체 프로세스 오케스트레이션

---

## 🎯 핵심 요약

**Claude의 역할**: 메인 에이전트로서 전체 평가 프로세스 오케스트레이션
**총 수집량**: **1,000개** (카테고리당 100개 × 10개 카테고리)
**총 평가량**: **4,000개** (4개 평가 AI × 1,000개)

---

## ⚠️ 중요한 개념

### ⭐ 프로세스 시작점

**잘못된 이해**:
```
Step 1: 수집 시작 (정치인 이름만 가지고)
```

**올바른 이해**:
```
Step 0: 정치인 정보 수집 (필수 선행 단계!)
  → 이름, 소속 정당, 출마 직종, 출마 지역, 나이 등 확인

Step 1: 수집 시작 (풍부한 컨텍스트를 담은 프롬프트로)
```

**왜 Step 0이 중요한가?**
- 프롬프트에 정치인의 상세 정보 포함 필요
- 동명이인 구분 필요
- 수집 및 평가의 정확도 향상
- 컨텍스트 풍부화로 AI 판단력 향상

---

## 📋 전체 프로세스 (7단계)

### Step 0: 정치인 정보 수집 ⭐ (가장 중요!)

**목적**: 평가 대상 정치인의 상세 정보 확보

**작업 내용**:
```python
# Supabase에서 정치인 정보 조회
from supabase import create_client, Client

politician_id = '62e7b453'  # 예: 오세훈
response = supabase.table('politicians').select('*').eq('id', politician_id).execute()

politician_data = response.data[0]
```

**확보해야 할 정보**:
```python
{
  "id": "62e7b453",
  "name": "오세훈",
  "party": "국민의힘",
  "title": "광역단체장",           # 출마 직종
  "position": "서울특별시장",       # 현재 직책
  "region": "서울특별시",          # 출마 지역
  "district": null,
  "identity": "출마예정자",         # 출마 신분
  "birth_date": "1961-08-04",
  "age": 64,
  "profile_image_url": "...",
  "website_url": "..."
}
```

**검증 체크리스트**:
- [ ] 정치인 ID가 올바른가? (8자리 hex TEXT)
- [ ] 이름이 정확한가?
- [ ] 소속 정당이 확인되었는가?
- [ ] 출마 직종(title)이 설정되어 있는가?
- [ ] 출마 지역(region)이 확인되었는가?
- [ ] 나이 정보가 있는가?

**사용자에게 확인 요청**:
```
정치인 정보를 확인했습니다:

이름: 오세훈
소속 정당: 국민의힘
출마 직종: 광역단체장
출마 지역: 서울특별시
나이: 64세
출마 신분: 출마예정자

이 정보로 평가를 진행하시겠습니까?
```

---

### Step 1: 데이터 수집 (1,000개)

**목적**: 3개 AI가 분담하여 총 1,000개 데이터 수집

**수집 AI 분담** (비용 최적화):

| AI | 비율 | 카테고리당 | 전체 (10 카테고리) | 특화 영역 |
|----|------|------------|-------------------|-----------|
| Gemini | 60% | 60개 | 600개 | 공식(50) + 공개(10) |
| Perplexity | 30% | 30개 | 300개 | 뉴스/언론 전담 |
| Grok | 10% | 10개 | 100개 | X/트위터 전담 |
| **합계** | **100%** | **100개** | **1,000개** | - |

⚠️ **Claude/ChatGPT는 수집에서 제외** (웹검색 비용 문제)

**수집 구조** (카테고리당 100개):
```
Phase 1: 부정 주제 20개 (최소 20% 보장)
├── OFFICIAL 부정 10개 (공식 데이터, 2022-01-14 ~ 2026-01-14)
└── PUBLIC 부정 10개 (공개 데이터, 2024-01-14 ~ 2026-01-14)

Phase 2: 긍정 주제 20개 (최소 20% 보장)
├── OFFICIAL 긍정 10개 (공식 데이터, 2022-01-14 ~ 2026-01-14)
└── PUBLIC 긍정 10개 (공개 데이터, 2024-01-14 ~ 2026-01-14)

Phase 3: 자유 평가 60개
├── OFFICIAL 자유 30개 (공식 데이터, 2022-01-14 ~ 2026-01-14)
└── PUBLIC 자유 30개 (공개 데이터, 2024-01-14 ~ 2026-01-14)

결과: OFFICIAL 50개 (50%) + PUBLIC 50개 (50%)
```

**10개 카테고리**:
1. 전문성 (expertise)
2. 리더십 (leadership)
3. 비전 (vision)
4. 청렴성 (integrity)
5. 윤리성 (ethics)
6. 책임감 (accountability)
7. 투명성 (transparency)
8. 소통능력 (communication)
9. 대응성 (responsiveness)
10. 공익성 (publicinterest)

**실행 방법**:
```bash
# V30 수집 스크립트 실행
python 0-3_AI_Evaluation_Engine/설계문서_V7.0/실행스크립트/collect_v30.py \
  --politician_id=62e7b453 \
  --politician_name="오세훈"
```

**프롬프트 예시** (Step 0에서 얻은 정보 활용):
```markdown
# 수집 대상 정치인
이름: 오세훈
소속 정당: 국민의힘
출마 직종: 광역단체장
출마 지역: 서울특별시
나이: 64세
현재 직책: 서울특별시장

# 수집 카테고리: 전문성
# 수집 항목: 10개 구체적 항목 (학력, 자격증, 경력, ...)

# 기간 제한
OFFICIAL 데이터: 2022-01-14 ~ 2026-01-14
PUBLIC 데이터: 2024-01-14 ~ 2026-01-14

# 수집 지침
Phase 1: 부정 주제 20개
Phase 2: 긍정 주제 20개
Phase 3: 자유 평가 60개
```

**수집 결과 저장**:
```
DB 테이블: collected_data_v30
레코드 수: 1,000개 (100개 × 10개 카테고리)
```

---

### Step 2: 풀링 (Pooling)

**목적**: 3개 AI가 수집한 데이터를 카테고리별로 통합

**프로세스**:
```python
# 카테고리별로 수집된 데이터 조회
for category in categories:
    pool = supabase.table('collected_data_v30') \
        .select('*') \
        .eq('politician_id', politician_id) \
        .eq('category', category) \
        .execute()

    # 결과: 카테고리당 100개 풀 (Gemini 60 + Perplexity 30 + Grok 10)
```

**중요 특징**:
- ❌ 중복 제거 안 함 (있는 그대로 합침)
- ❌ 가중치 계산 안 함 (자연 가중치 활용)
- ✅ 4개 AI가 동일 뉴스 수집 → 풀에 4번 포함 → 자연스럽게 4배 영향

**자연 가중치 예시**:
```
뉴스 A: Gemini만 수집 → 풀에 1번 포함 → 평가 시 1배 영향
뉴스 B: 3개 AI 모두 수집 → 풀에 3번 포함 → 평가 시 3배 영향
```

---

### Step 3: 평가 (Evaluation)

**목적**: 4개 AI가 각각 전체 풀(1,000개)을 평가

**평가 AI** (4개):
- Claude (me!)
- ChatGPT
- Gemini
- Grok

⚠️ **Perplexity는 평가에서 제외** (수집 전담)

**평가 방식** (AI당):
```
카테고리당 100개 × 10개 카테고리 = 1,000개 평가
배치 처리: 10개씩 나눠서 100번 평가 호출
```

**전체 평가량**:
```
4개 AI × 1,000개 = 4,000개 평가 레코드
```

**등급 체계** (V30 - 9단계):
```
+4: 매우 긍정적 (탁월함)
+3: 긍정적 (우수함)
+2: 다소 긍정적 (양호함)
+1: 약간 긍정적 (경미한 긍정)
 0: 중립 (보통)
-1: 약간 부정적 (경미한 부정)
-2: 다소 부정적 (미흡함)
-3: 부정적 (불량함)
-4: 매우 부정적 (매우 불량함)
```

**실행 방법**:
```bash
# V30 평가 스크립트 실행
python 0-3_AI_Evaluation_Engine/설계문서_V7.0/실행스크립트/evaluate_v30.py \
  --politician_id=62e7b453 \
  --politician_name="오세훈"
```

**타이밍 분리** (객관성 보장):
```
수집 시점 ≠ 평가 시점 (다른 세션)
→ 독립적 판단 = 더 객관적
```

**평가 결과 저장**:
```
DB 테이블: evaluations_v30
레코드 수: 4,000개 (4개 AI × 1,000개)
```

---

### Step 4: AI별 카테고리 점수 계산

**목적**: 각 AI가 각 카테고리에 대해 부여한 평균 점수 계산

**계산 방식**:
```python
# AI별, 카테고리별 평균 등급 계산
for ai in evaluation_ais:  # Claude, ChatGPT, Gemini, Grok
    for category in categories:  # 10개 카테고리
        ratings = supabase.table('evaluations_v30') \
            .select('rating') \
            .eq('politician_id', politician_id) \
            .eq('ai_name', ai) \
            .eq('category', category) \
            .execute()

        avg_rating = sum(ratings) / len(ratings)  # -4 ~ +4

        # 점수 변환 (20~100점)
        PRIOR = 6.0
        COEFFICIENT = 0.5
        category_score = (PRIOR + avg_rating * COEFFICIENT) * 10
```

**점수 범위**:
```
평균 등급: -4 ~ +4
카테고리 점수: 20 ~ 100점

계산식:
score = (6.0 + avg_rating * 0.5) * 10

예시:
avg_rating = +4 → score = (6.0 + 2.0) * 10 = 80점
avg_rating =  0 → score = (6.0 + 0.0) * 10 = 60점
avg_rating = -4 → score = (6.0 - 2.0) * 10 = 40점
```

**저장**:
```
DB 테이블: ai_category_scores_v30
레코드 수: 40개 (4개 AI × 10개 카테고리)
```

---

### Step 5: AI별 최종 점수 계산

**목적**: 각 AI의 10개 카테고리 점수를 합산하여 최종 점수 산출

**계산 방식**:
```python
for ai in evaluation_ais:
    category_scores = supabase.table('ai_category_scores_v30') \
        .select('score') \
        .eq('politician_id', politician_id) \
        .eq('ai_name', ai) \
        .execute()

    total_score = sum(category_scores)
    final_score = min(total_score, 1000)  # 최대 1000점
```

**점수 범위**:
```
최소: 200점 (20점 × 10개 카테고리)
최대: 1000점 (100점 × 10개 카테고리)
```

**저장**:
```
DB 테이블: ai_final_scores_v30
레코드 수: 4개 (4개 AI)
```

---

### Step 6: 통합 점수 계산

**목적**: 4개 AI의 최종 점수를 평균하여 정치인의 통합 점수 산출

**계산 방식**:
```python
ai_scores = supabase.table('ai_final_scores_v30') \
    .select('final_score') \
    .eq('politician_id', politician_id) \
    .execute()

integrated_score = round(sum(ai_scores) / len(ai_scores))
```

**업데이트**:
```python
supabase.table('politicians') \
    .update({'integrated_score': integrated_score}) \
    .eq('id', politician_id) \
    .execute()
```

---

### Step 7: 결과 보고 및 검증

**보고 내용**:
```markdown
# V30 평가 완료: 오세훈

## 수집 결과
- 총 수집량: 1,000개
- Gemini: 600개 (60%)
- Perplexity: 300개 (30%)
- Grok: 100개 (10%)

## 평가 결과
- 총 평가량: 4,000개 (4개 AI × 1,000개)
- Claude 평가: 1,000개
- ChatGPT 평가: 1,000개
- Gemini 평가: 1,000개
- Grok 평가: 1,000개

## AI별 최종 점수
- Claude: 720점
- ChatGPT: 735점
- Gemini: 710점
- Grok: 725점

## 통합 점수
- 평균: 723점

## 카테고리별 점수 (Claude 기준)
1. 전문성: 75점
2. 리더십: 78점
3. 비전: 70점
4. 청렴성: 68점
5. 윤리성: 72점
6. 책임감: 74점
7. 투명성: 71점
8. 소통능력: 76점
9. 대응성: 73점
10. 공익성: 69점
```

**검증 체크리스트**:
- [ ] 수집량이 1,000개인가?
- [ ] 평가량이 4,000개인가?
- [ ] 4개 AI 모두 평가했는가?
- [ ] 카테고리별 점수가 20~100 범위인가?
- [ ] 최종 점수가 200~1000 범위인가?
- [ ] DB에 모든 데이터가 저장되었는가?

---

## 🔧 실행 스크립트

### 전체 프로세스 자동화

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
V30 전체 프로세스 오케스트레이션 스크립트
"""

import os
import sys
from supabase import create_client, Client

# Supabase 설정
SUPABASE_URL = os.getenv("SUPABASE_URL")
SUPABASE_KEY = os.getenv("SUPABASE_SERVICE_ROLE_KEY")
supabase: Client = create_client(SUPABASE_URL, SUPABASE_KEY)

def step_0_gather_politician_info(politician_id: str):
    """Step 0: 정치인 정보 수집"""
    print(f"\n[Step 0] 정치인 정보 수집 중...")

    response = supabase.table('politicians').select('*').eq('id', politician_id).execute()

    if not response.data:
        raise ValueError(f"정치인을 찾을 수 없습니다: {politician_id}")

    politician = response.data[0]

    print(f"\n정치인 정보:")
    print(f"  이름: {politician['name']}")
    print(f"  소속 정당: {politician['party']}")
    print(f"  출마 직종: {politician['title']}")
    print(f"  출마 지역: {politician['region']}")
    print(f"  나이: {politician['age']}세")
    print(f"  출마 신분: {politician['identity']}")

    # 필수 정보 검증
    required_fields = ['name', 'party', 'title', 'region', 'identity']
    for field in required_fields:
        if not politician.get(field):
            raise ValueError(f"필수 정보 누락: {field}")

    return politician

def step_1_collect_data(politician_id: str, politician_name: str):
    """Step 1: 데이터 수집 (1,000개)"""
    print(f"\n[Step 1] 데이터 수집 시작...")

    # collect_v30.py 스크립트 실행
    os.system(f'python 0-3_AI_Evaluation_Engine/설계문서_V7.0/실행스크립트/collect_v30.py --politician_id={politician_id} --politician_name="{politician_name}"')

    # 수집 결과 확인
    response = supabase.table('collected_data_v30').select('id').eq('politician_id', politician_id).execute()
    collected_count = len(response.data)

    print(f"  수집 완료: {collected_count}개")

    if collected_count != 1000:
        raise ValueError(f"수집량 오류: 예상 1,000개, 실제 {collected_count}개")

    return collected_count

def step_3_evaluate_data(politician_id: str, politician_name: str):
    """Step 3: 데이터 평가 (4,000개)"""
    print(f"\n[Step 3] 데이터 평가 시작...")

    # evaluate_v30.py 스크립트 실행
    os.system(f'python 0-3_AI_Evaluation_Engine/설계문서_V7.0/실행스크립트/evaluate_v30.py --politician_id={politician_id} --politician_name="{politician_name}"')

    # 평가 결과 확인
    response = supabase.table('evaluations_v30').select('id').eq('politician_id', politician_id).execute()
    evaluated_count = len(response.data)

    print(f"  평가 완료: {evaluated_count}개")

    if evaluated_count != 4000:
        raise ValueError(f"평가량 오류: 예상 4,000개, 실제 {evaluated_count}개")

    return evaluated_count

def step_4_5_6_calculate_scores(politician_id: str):
    """Step 4-6: 점수 계산"""
    print(f"\n[Step 4-6] 점수 계산 시작...")

    # calculate_v30_scores.py 스크립트 실행
    os.system(f'python 0-3_AI_Evaluation_Engine/설계문서_V7.0/실행스크립트/calculate_v30_scores.py --politician_id={politician_id}')

    # 최종 점수 확인
    response = supabase.table('politicians').select('integrated_score').eq('id', politician_id).execute()
    integrated_score = response.data[0]['integrated_score']

    print(f"  통합 점수: {integrated_score}점")

    return integrated_score

def main():
    """메인 함수"""
    if len(sys.argv) < 2:
        print("사용법: python orchestrate_v30.py <politician_id>")
        sys.exit(1)

    politician_id = sys.argv[1]

    try:
        # Step 0: 정치인 정보 수집
        politician = step_0_gather_politician_info(politician_id)
        politician_name = politician['name']

        # 사용자 확인
        print(f"\n위 정보로 평가를 진행하시겠습니까? (y/n): ", end='')
        if input().lower() != 'y':
            print("평가를 취소합니다.")
            sys.exit(0)

        # Step 1: 데이터 수집
        collected_count = step_1_collect_data(politician_id, politician_name)

        # Step 2: 풀링 (자동)
        print(f"\n[Step 2] 풀링 (자동 완료)")

        # Step 3: 데이터 평가
        evaluated_count = step_3_evaluate_data(politician_id, politician_name)

        # Step 4-6: 점수 계산
        integrated_score = step_4_5_6_calculate_scores(politician_id)

        # Step 7: 결과 보고
        print(f"\n" + "="*80)
        print(f"V30 평가 완료: {politician_name}")
        print(f"="*80)
        print(f"총 수집량: {collected_count}개")
        print(f"총 평가량: {evaluated_count}개")
        print(f"통합 점수: {integrated_score}점")
        print(f"="*80)

    except Exception as e:
        print(f"\n❌ 오류 발생: {e}")
        sys.exit(1)

if __name__ == "__main__":
    main()
```

---

## 💡 피해야 할 실수 (Learning from Errors)

### 실수 1: Step 0 누락

**잘못된 시작**:
```
Step 1: 수집 시작 (이름만 가지고)
```

**올바른 시작**:
```
Step 0: 정치인 정보 수집 (필수!)
  → 이름, 정당, 직종, 지역, 나이 등 확인

Step 1: 수집 시작 (풍부한 컨텍스트로)
```

**원인**: 프롬프트에 필요한 상세 정보를 간과

---

### 실수 2: politician_id 타입 오류

**잘못된 처리**:
```python
politician_id = parseInt(params.id)  # ❌ NaN 발생
```

**올바른 처리**:
```python
politician_id = params.id  # ✅ 문자열 그대로
```

**원인**: politician_id는 **TEXT 타입** (8자리 hex), 숫자 아님!

---

## 📚 참조 문서

- `V30_검증요청서.md` - V30 시스템 검증 절차
- `V30_검증결과_독립검증_2026-01-17.md` - 독립 검증 결과
- `V30_기본방침.md` - V30 시스템 기본 방침
- `V30_전체_프로세스_가이드.md` - 전체 프로세스 가이드
- `2_collect/` - 10개 수집 지침서
- `3_evaluate/` - 10개 평가 지침서
- `실행스크립트/collect_v30.py` - 수집 스크립트
- `실행스크립트/evaluate_v30.py` - 평가 스크립트
- `실행스크립트/calculate_v30_scores.py` - 점수 계산 스크립트

---

## ✅ 체크리스트

### 시작 전
- [ ] 정치인 ID 확인 (8자리 hex TEXT)
- [ ] DB 연결 확인 (Supabase)
- [ ] API 키 설정 확인 (Gemini, Perplexity, Grok, Claude, ChatGPT)
- [ ] Step 0 완료 (정치인 정보 수집)

### 수집 중
- [ ] 3개 AI 분담 확인 (60-30-10)
- [ ] 기간 제한 적용 확인
- [ ] 카테고리당 100개 확인
- [ ] 총 1,000개 수집 확인

### 평가 중
- [ ] 4개 AI 평가 확인 (Claude, ChatGPT, Gemini, Grok)
- [ ] Perplexity 평가 제외 확인
- [ ] 카테고리당 100개 평가 확인
- [ ] 총 4,000개 평가 확인

### 완료 후
- [ ] 통합 점수 200~1000 범위 확인
- [ ] DB 저장 확인
- [ ] 결과 보고 작성
- [ ] 사용자 승인

---

**작성자**: Claude Code (Session 2026-01-17)
**검증**: 사용자 피드백 기반 수정
**버전**: V30 Final

---

**🚨 핵심 기억 사항**:
1. **1,000개 수집** (60% + 30% + 10% = 100%)
2. **Step 0 필수**: 정치인 정보 먼저 수집!
3. **politician_id는 TEXT**: parseInt() 금지!
