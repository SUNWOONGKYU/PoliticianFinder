...
ITEMS = [
    {
        'item_num': 1,
        'name': '사회복지 예산 비율',
        'description': '(복지 예산 / 전체 예산) × 100',
        'data_source': '지방재정365',
        'type': 'Official'
    },
    {
        'item_num': 2,
        'name': '취약계층 지원 프로그램 건수',
        'description': '장애·노인·아동·저소득 지원 사업 수',
        'data_source': '지자체/교육청 사업 공고',
        'type': 'Official'
    },
    {
        'item_num': 3,
        'name': '환경·기후 예산 비율 또는 증가율',
        'description': '(환경 예산 / 전체) × 100 또는 증가율',
        'data_source': '지방재정365',
        'type': 'Official'
    },
    {
    ...
    },
    {
        'item_num': 5,
        'name': '공익 활동 언론 보도 건수',
        'description': '"봉사", "취약계층", "복지" 키워드 보도',
        'data_source': '빅카인즈',
        'type': 'Public'
    },
    {
        'item_num': 6,
        'name': '사회공헌 SNS 게시물 비중',
        'description': '(공익 게시물 / 전체 게시물) × 100',
        'data_source': '공개 SNS',
        'type': 'Public'
    },
    {
...
def insert_data_point(conn, politician_id, item_num, data_point):
    """단일 데이터 포인트 DB 삽입"""
    cursor = conn.cursor()
    try:
        cursor.execute("""
            INSERT INTO collected_data (
                ...
                rating, reliability, collected_at
...
def main():
    ...
    try:
        ...
        for item in ITEMS:
            ...
            # 항목별 데이터 수집 함수 호출
            if item_num == 1:
                data_points = collect_item_1_data()
            elif item_num == 2:
                data_points = collect_item_2_data()
            elif item_num == 3:
                data_points = collect_item_3_data()
            elif item_num == 4:
                data_points = collect_item_4_data()
            elif item_num == 5:
                data_points = collect_item_5_data()
            elif item_num == 6:
                data_points = collect_item_6_data()
            elif item_num == 7:
                data_points = collect_item_7_data()
            else:
                data_points = []
            ...
            for dp in data_points:
                try:
                    insert_data_point(conn, politician_uuid, item_num, dp)
                    inserted_count += 1
                except Exception as e:
                    print(f"  - 데이터 삽입 실패: {e}")
        ...
        if results:
            total_rating = sum(r[3] for r in results if r[3] is not None)
