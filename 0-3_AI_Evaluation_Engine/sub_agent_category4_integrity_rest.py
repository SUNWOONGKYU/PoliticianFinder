...
# -*- coding: utf-8 -*-
"""
서브 에이전트 - 카테고리 4 (청렴성) 평가 및 DB 저장 (REST API)
정치인: 오세훈
카테고리: 청렴성
"""
...
def insert_data_batch(politician_uuid, data_list):
    """데이터 배치 삽입"""
    ...
    for data_point in data_list:
        record = {
            ...
            'item_num': data_point['item_num'],
            'data_source': data_point.get('data_type', '언론보도'),
            'data_title': data_point['title'],
            'data_content': data_point['content'],
            'source_url': data_point['url'],
            'rating': data_point['rating'],
            'rating_rationale': f"{data_point['title']}에 대한 평가",
            'reliability': data_point.get('reliability', 0.85),
    ...
    if response.status_code not in [200, 201]:
        print(f"  DB 삽입 오류: {response.status_code}")
...
def get_statistics(politician_uuid):
    """결과 통계 조회"""
    url = f"{SUPABASE_URL}/rest/v1/rpc/get_category_stats"
    headers = {
        'apikey': SUPABASE_KEY,
        'Authorization': f'Bearer {SUPABASE_KEY}',
        'Content-Type': 'application/json'
    ...
    if response.status_code == 200:
        ...
        for item in data:
            ...
            if item_num not in stats:
                stats[item_num] = {
                    'count': 0,
                    'total_rating': 0,
                    'total_reliability': 0
                }
            stats[item_num]['count'] += 1
            stats[item_num]['total_rating'] += item['rating']
            stats[item_num]['total_reliability'] += item['reliability']

        # 평균 계산
        for item_num in stats:
            stats[item_num]['avg_rating'] = stats[item_num]['total_rating'] / stats[item_num]['count']
            stats[item_num]['avg_reliability'] = stats[item_num]['total_reliability'] / stats[item_num]['count']
...
def main():
    ...
    try:
        # 정치인 UUID 조회
        print(f"[1/3] 정치인 '{POLITICIAN_NAME}' UUID 조회 중...")
        ...
        # DB 저장
        print("[3/3] DB에 데이터 저장 중...")
        success = insert_data_batch(politician_uuid, all_data)
        if success:
            print("  DB 저장 성공!")
        else:
            print("  DB 저장 실패!")
        ...
        stats, raw_data = get_statistics(politician_uuid)
        if stats:
            total_data = len(raw_data)
            avg_rating = sum(d['rating'] for d in raw_data) / total_data if total_data > 0 else 0
            ...
            for item_num in sorted(stats.keys()):
                stat = stats[item_num]
                print(f"4-{item_num:<8} {stat['count']:<12} {stat['avg_rating']:<15.2f} {stat['avg_reliability']:<15.2f}")
