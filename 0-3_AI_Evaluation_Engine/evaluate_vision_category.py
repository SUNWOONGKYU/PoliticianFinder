...
def get_politician_uuid(conn, politician_id):
    """정치인 ID(숫자)를 UUID로 변환 또는 조회"""
    cursor = conn.cursor()
    ...
    cursor.execute("""
        ...
        VALUES (%s, %s, %s, %s, %s)
        RETURNING id
    """, (POLITICIAN_NAME, '광역단체장', '국민의힘', '서울특별시', '서울특별시장'))

    conn.commit()
    return cursor.fetchone()[0]


def insert_collected_data(conn, politician_uuid):
    """수집된 데이터를 DB에 삽입"""
    cursor = conn.cursor()
    total_inserted = 0
    ...
    for item_num, data_list in VISION_DATA.items():
        ...
        # 항목별 commit
        conn.commit()
        print(f"  완료: {len(data_list)}개 삽입")

    return total_inserted


def verify_results(conn, politician_uuid):
    """결과 확인"""
    cursor = conn.cursor()
    ...
    for row in results:
        cat_num, item_num, count, avg_rating = row
    ...
    overall_avg = sum(r[3] for r in results) / len(results) if results else 0
    ...
    if scores:
        ...
        print("="*80)
        for row in scores:
            item_num, item_score, rating_avg, data_count = row
            item_name = VISION_ITEMS[item_num]['name']
            print(f"항목 {item_num}: {item_name}")
    ...
    if category_result:
        cat_score, items_completed = category_result
        print("\n📊 자동 계산된 분야 점수 (AI Category Score)")
        print("="*80)
        print(f"카테고리 {CATEGORY_NUM}: {CATEGORY_NAME}")
        print(f"  - 분야 점수: {cat_score:.2f}/100.0")
        print(f"  - 완료 항목: {items_completed}/7개")
...
def main():
    ...
    try:
        ...
        # 데이터 삽입
        print(f"\n비전 카테고리 데이터 수집 시작...")
        total_inserted = insert_collected_data(conn, politician_uuid)

        print(f"\n✅ 데이터 삽입 완료: 총 {total_inserted}개")

        # 결과 확인
        verify_results(conn, politician_uuid)
