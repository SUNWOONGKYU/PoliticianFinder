...
## 2026-01-13 00:00 - 출마 신분 체계 정리 및 지역 정보 수정

**문제**:
1. 출마 신분별에 "현직" 2명이 있음 (실제로는 모두 출마예정자)
2. 출마 신분이 4가지 (현직, 후보자, 예비후보자, 출마예정자)
3. 정원오 출마지역: "서울특별시 성동구"로 표시

**사용자 요구사항**:
1. 출마 신분은 3가지만: 출마예정자, 예비후보자, 후보자
2. "현직" 항목 삭제
3. 모든 정치인은 출마예정자
4. 정원오 출마지역: "서울특별시"로만 표시

**DB 수정** (`update_identity_and_region.py`):
...
2. **정원오** (ID: 17270f25)
   - identity: "현직" → "출마예정자"
   - region: "서울특별시 성동구" → "서울특별시"

**API 수정** (`sidebar/route.ts`):
```typescript
// Before
const identityStats = {
  현직: 0,
...
1. 타입 정의에서 "현직" 제거
2. UI 표시 순서 변경:
   - Before: 현직 → 후보자 → 예비후보자 → 출마예정자
   - After: 출마예정자 → 예비후보자 → 후보자
