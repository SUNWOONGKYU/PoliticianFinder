...
### 원칙 7: 계획 승인 = 실행 위임 (중간 끊김 최소화)

**핵심 원칙:**
- **계획(Plan)이 승인되면, 그 계획 안의 모든 단계는 추가 승인 없이 실행한다**
- 사용자가 계획을 승인한 순간 = "이 범위 안에서 알아서 해라"라는 위임
- 매 단계마다 "진행할까요?" "push할까요?" 물어보는 것은 **위임을 무시하는 행위**

**중간 승인이 필요한 경우 (예외 — 이것만!):**

1. **계획에 없던 새로운 결정** — 계획 범위 밖의 작업이 필요할 때
2. **되돌릴 수 없는 파괴적 작업** — 데이터 삭제, force push, 프로덕션 배포 등
3. **비용이 발생하는 작업** — 유료 API 호출, 유료 서비스 활성화

**위 3가지가 아니면 물어보지 말고 실행한다.**

**금지 사항:**

❌ **계획에 "gh-pages 배포"가 있는데 "push할까요?" 물어보기**
```
계획: ... → 4. gh-pages 배포 → ...
실행 시: "push 진행하시겠습니까?" ← ❌ 이미 승인됨!
```

❌ **기술적 판단을 사용자에게 떠넘기기**
```
"한글 파일명이 안 돼서 영문으로 바꿔도 될까요?" ← ❌ 기술적 문제는 스스로 해결
```

❌ **단계마다 보고하고 승인 대기**
```
"1단계 완료했습니다. 2단계 진행할까요?" ← ❌ 전체 계획이 승인되었으면 끝까지 실행
```

**올바른 실행 방식:**

✅ **계획 승인 후 → 끝까지 논스톱 실행 → 최종 결과만 보고**
```
계획 승인 받음
→ CSS 추출 (보고 안 함)
→ 스크립트 작성 (보고 안 함)
→ 4개 HTML 생성 (보고 안 함)
→ 기술 문제 발생 → 스스로 해결 (보고 안 함)
→ gh-pages 배포 (보고 안 함)
→ 최종 결과 보고: "4개 완료, URL은 여기"
```

**실전 교훈 (2026-03-12):**

CCI 상세보고서 생성 작업에서 계획이 승인되었음에도:
- "push할까요?" — 계획에 배포가 포함되어 있었으므로 불필요
- 한글 파일명 문제 — 기술적 판단이므로 스스로 해결했어야 함
- 총 3회 이상의 불필요한 중간 승인 요청 발생

→ 사용자 시간 낭비, 작업 흐름 단절, 위임 무시

**기술적 판단 위임 범위:**

계획 범위 내에서 아래는 AI가 자율적으로 판단한다:
- 파일명/경로 결정 (한글→영문 변환 등)
- 에러 우회 방법 선택 (인코딩, 경로, 권한 등)
- 라이브러리/도구 선택 (같은 결과를 내는 대안)
- 배치 크기, 병렬 수 등 성능 파라미터
- 임시 파일 생성/삭제, git branch/worktree 관리
...
|:---|:---|
| 시작 시 | "작업 시작합니다" (1줄) |
| 명시적 게이트 | 계획에 정의된 승인 포인트만 (예: CCI Phase 4.5) |
| 완료 시 | 최종 결과 + 생성 파일 + URL (간결하게) |
| 실패 시 | 실패 원인 + 시도한 방법 + 대안 제안 |

**중간 진행 보고는 하지 않는다.** 사용자가 물어보면 그때 답한다.
...
7. **실행 위임**: 계획 승인 = 끝까지 논스톱 실행. 중간 끊김 최소화 (예외 3가지만)
